...
public:
	...
	void MsgFunc_ScreenOverlay( bf_read & msg);
...
private:
	...
	CPanelAnimationVar( Color, m_DevourerFullscreenColor, "DevourerFullscreenColor", "255 0 0 0");
	CPanelAnimationVar( Color, m_GibFullscreenColor, "GibFullscreenColor", "255 0 0 0");

	void DrawDamageIndicator(int side);
	void DrawFullscreenDamageIndicator();
	void DrawScreenAnimations();
	void GetDamagePosition( const Vector &vecDelta, float *flRotation );

	// Mother of all overlay drawing methods
	void DrawOverlay(Color col, CMaterialReference & ref);

	CMaterialReference m_WhiteAdditiveMaterial;
	CMaterialReference m_BloodMaterial;
	CMaterialReference m_GibMaterial;
	CMaterialReference m_DevourerMaterial;
};

DECLARE_HUDELEMENT( CHudDamageIndicator );
DECLARE_HUD_MESSAGE( CHudDamageIndicator, Damage );
DECLARE_HUD_MESSAGE( CHudDamageIndicator, ScreenOverlay );
...
static const char * g_ScreenAnimations[] = 
{
	"HudGibFullscreen",
	"HudDevourerOmNomNomNom",
};
...
CHudDamageIndicator::CHudDamageIndicator( const char *pElementName ) : CHudElement( pElementName ), BaseClass(NULL, "HudDamageIndicator")
{
	...
	m_BloodMaterial.Init("effects/blood_full", TEXTURE_GROUP_CLIENT_EFFECTS);
	m_GibMaterial.Init("effects/blood_gib", TEXTURE_GROUP_CLIENT_EFFECTS);
	m_DevourerMaterial.Init("effects/blood_devourer", TEXTURE_GROUP_CLIENT_EFFECTS);
...
void CHudDamageIndicator::Init( void )
{
	...
	HOOK_HUD_MESSAGE( CHudDamageIndicator, ScreenOverlay );
}
...
bool CHudDamageIndicator::ShouldDraw( void )
{
	bool bNeedsDraw = m_DmgColorLeft[3] || 
						...
						m_DevourerFullscreenColor[3] ||
						m_GibFullscreenColor[3];
...
{
	DrawOverlay(m_DmgFullscreenColor, m_BloodMaterial);
}

void CHudDamageIndicator::DrawScreenAnimations()
{
	if(m_GibFullscreenColor[3])
	{
		DrawOverlay(m_GibFullscreenColor, m_GibMaterial);
	}

	if(m_DevourerFullscreenColor[3])
	{
		DrawOverlay(m_DevourerFullscreenColor, m_DevourerMaterial);
	}
}

void CHudDamageIndicator::DrawOverlay(Color col, CMaterialReference & ref)
{
	CMatRenderContextPtr pRenderContext( materials );
	IMesh *pMesh = pRenderContext->GetDynamicMesh( true, NULL, NULL, ref );

	CMeshBuilder meshBuilder;
	meshBuilder.Begin( pMesh, MATERIAL_QUADS, 1 );
	int r = col[0], g = col[1], b = col[2], a = col[3];
...
void CHudDamageIndicator::MsgFunc_ScreenOverlay(bf_read & msg)
{
	g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(g_ScreenAnimations[msg.ReadShort()]);
}
