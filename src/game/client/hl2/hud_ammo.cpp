...
{
	SetHiddenBits( HIDEHUD_HEALTH | HIDEHUD_PLAYERDEAD | HIDEHUD_NEEDSUIT | HIDEHUD_WEAPONSELECTION | HIDEHUD_CAMERA | HIDEHUD_OVBETA );
...
void CHudAmmo::UpdatePlayerAmmo( C_BasePlayer *player )
{
	...
	if ( !wpn || !player || !wpn->UsesPrimaryAmmo() || wpn->IsOverlordWeapon() )
	{
...
public:
	CHudSecondaryAmmo( const char *pElementName ) : BaseClass( NULL, "HudAmmoSecondary" ), CHudElement( pElementName )
	{
		...
		SetHiddenBits( HIDEHUD_HEALTH | HIDEHUD_WEAPONSELECTION | HIDEHUD_PLAYERDEAD | HIDEHUD_NEEDSUIT | HIDEHUD_CAMERA);
	}
