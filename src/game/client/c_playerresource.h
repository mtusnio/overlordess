...
public : // IGameResources intreface
	...
	virtual bool	IsAlive( int index ) const;
...
//	virtual int		GetPacketloss( int index );
	...
	virtual PlayerClass_t GetPlayerClass(int index);
	virtual const char *  GetPlayerClassName(int index);

	virtual bool    WantsToBeOverlord(int index) { return m_bWantsOverlord[index]; };
...
protected:
	...
	bool    m_bWantsOverlord[MAX_PLAYERS+1];
