...
void CEntGlowEffect::Init( void ) 
{
	...
	m_BlurX.Init( materials->FindMaterial("glow/ge_blurx", TEXTURE_GROUP_OTHER, true) );
	m_BlurY.Init( materials->FindMaterial("glow/ge_blury", TEXTURE_GROUP_OTHER, true) );
}
...
//-----------------------------------------------------------------------------
ConVar cl_ge_glowscale( "cl_ge_glowscale", "0.6", FCVAR_CLIENTDLL );
...
void CEntGlowEffect::RenderToGlowTexture( int idx, IMatRenderContext *pRenderContext )
{
	...
	const float * color = m_vGlowEnts[idx]->m_fColor;
...
Color CEntGlowEffect::GetEntColor( EHANDLE hEnt ) const
{
	int idx = FindGlowEnt(hEnt);
	if(idx == -1)
		return Color(0, 0, 0, 0);

	return Color(m_vGlowEnts[idx]->m_fColor[0], m_vGlowEnts[idx]->m_fColor[1],
		m_vGlowEnts[idx]->m_fColor[2], m_vGlowEnts[idx]->m_fColor[3]);
}

float CEntGlowEffect::GetEntGlowScale( EHANDLE hEnt ) const
{
	int idx = FindGlowEnt(hEnt);
	if(idx == -1)
		return 0.0f;

	return m_vGlowEnts[idx]->m_fGlowScale;
}
 
int CEntGlowEffect::FindGlowEnt( EHANDLE hEnt ) const
{
	for ( int i=0; i < m_vGlowEnts.Count(); i++ )
	{
