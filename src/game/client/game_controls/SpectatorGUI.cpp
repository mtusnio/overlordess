...
void CSpectatorGUI::Update()
{
	...
	wchar_t szEtxraInfo[512];
	
	wchar_t szTitleLabel[512];
	char tempstr[512];
	...
	{
		Q_snprintf(tempstr, ARRAYSIZE(tempstr), "Your points: %i", pPlayer->DamageToOverlord());
		g_pVGuiLocalize->ConvertANSIToUnicode(tempstr,szEtxraInfo,sizeof(szEtxraInfo));
	...
	//SetLabelText("extrainfo2", szExtraInfo2);
...
// Update dynamically-changing labels
void CSpectatorGUI::UpdateLabels()
{
	wchar_t szExtraInfo3[512];
	wchar_t szExtraInfo2[512];
	char tempstr[512];
	...
	int health = 0;
	int maxhealth = 0;
	if(pData && pData->GetOverlord())
	{
		health = (pData->GetOverlord()->GetHealth() >= 0) ? pData->GetOverlord()->GetHealth() : 0;

		if(pData->IsCountingHealth())
		{
			maxhealth = pData->GetHealthCap();
		}
		else
			maxhealth = pData->GetOverlord()->GetMaxHealth();

		
		Q_snprintf(tempstr, ARRAYSIZE(tempstr), "Overlordess health: %i/%i", health, maxhealth);
	}
	else
	{
		Q_strncpy(tempstr, "No Overlordess", ARRAYSIZE(tempstr));
	}
	g_pVGuiLocalize->ConvertANSIToUnicode(tempstr, szExtraInfo3, ARRAYSIZE(szExtraInfo3));

	SetLabelText("extrainfo3", szExtraInfo3);
	...
	{
		char key[32];
		Q_strncpy(key, engine->Key_LookupBinding("eo_respawn"), ARRAYSIZE(key));

		Q_snprintf(tempstr, ARRAYSIZE(tempstr), "You are ready to respawn. Press '%s'.", key);
	}
	else if(GetOverlordData()->GetSpawnsAmount() > 0)
	{
		Q_strncpy(tempstr, "You cannot respawn at this time.", ARRAYSIZE(tempstr));
	}
	else
	{
		Q_strncpy(tempstr, "No more spawns are left.", ARRAYSIZE(tempstr));
	}

	g_pVGuiLocalize->ConvertANSIToUnicode(tempstr, szExtraInfo2, ARRAYSIZE(szExtraInfo2));
