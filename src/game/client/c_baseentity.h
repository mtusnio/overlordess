...
public:
	...
	virtual bool					IsHL2MPPlayer( void ) const { return false; };
	...
	// Returns text for player target display
	//virtual bool					ShouldShowTargetText() const { return false; };
	virtual const char *            GetTargetText() const { return NULL; };
	virtual const char *            GetTargetTextFormat() const { return NULL; };

	// Returns the eye point + angles (used for viewing + shooting)
	virtual Vector			EyePosition( void );
	virtual Vector			RealEyePosition( void );
...
//	const QAngle&		GetAbsAngularVelocity( ) const;
	...
	void						RegisterGlow(Color clr, float flGlowScale = 1.0f);
	void						DeregisterGlow();
	bool						IsGlowRegistered() const { return m_bGlowRegistered; };
	void						SetGlowColor(Color clr);			
	Color						GetGlowColor();
#ifdef _DEBUG
...
public:
	...
	void SetRenderColor(Color clr) { SetRenderColor(clr.r(), clr.g(), clr.b(), clr.a()); };
