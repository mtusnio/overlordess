...
// Ov buttons
//static  kbutton_t   in_lockxaxis;
//static  kbutton_t   in_lockyaxis;
//static	kbutton_t	in_freeroam;
static  kbutton_t   in_shownodes;
static  kbutton_t   in_blink;
...
//void IN_LockXAxisUp(const CCommand &args ) { KeyUp( &in_lockxaxis, args[1]); };
//void IN_LockXAxisDown(const CCommand &args ) { KeyDown( &in_lockxaxis, args[1]); };
//void IN_LockYAxisUp(const CCommand &args ) { KeyUp( &in_lockyaxis, args[1]); };
//void IN_LockYAxisDown(const CCommand &args ) { KeyDown( &in_lockyaxis, args[1]); };
void IN_ShowNodesUp(const CCommand & args) { KeyUp( &in_shownodes, args[1]); };
void IN_ShowNodesDown(const CCommand & args) { KeyDown(&in_shownodes, args[1]); };

void IN_BlinkUp(const CCommand & args) { KeyUp( &in_blink, args[1]); };
void IN_BlinkDown(const CCommand & args) { KeyDown(&in_blink, args[1]); };
//void IN_FreeroamUp(const CCommand & args) { KeyUp( &in_freeroam, args[1]); };
//void IN_FreeroamDown(const CCommand & args) { KeyDown( &in_freeroam, args[1]); };
...
int CInput::GetButtonBits( int bResetState )
{
	...
	// Ov bits
	//CalcButtonBits( bits, IN_LOCKXAXIS, s_ClearInputState, &in_lockxaxis, bResetState );
	//CalcButtonBits( bits, IN_LOCKYAXIS, s_ClearInputState, &in_lockyaxis, bResetState );
	CalcButtonBits( bits, IN_BLINK, s_ClearInputState, &in_blink, bResetState);
	CalcButtonBits( bits, IN_SHOWNODES, s_ClearInputState, &in_shownodes, bResetState);
	//CalcButtonBits( bits, IN_FREEROAM, s_ClearInputState, &in_freeroam, bResetState );
...
// Ov commands
//static ConCommand startlockxaxis( "+lockxaxis", IN_LockXAxisDown );
//static ConCommand endlockxaxis( "-lockxaxis", IN_LockXAxisUp );

//static ConCommand startlockyaxis( "+lockyaxis", IN_LockYAxisDown );
//static ConCommand endlockyaxis( "-lockyaxis", IN_LockYAxisUp );

//static ConCommand startfreeroam( "+freeroam", IN_FreeroamDown);
//static ConCommand endfreeroam( "-freeroam", IN_FreeroamUp);

static ConCommand startshownodes("+shownodes", IN_ShowNodesDown);
static ConCommand stopshownodes("-shownodes", IN_ShowNodesUp);

static ConCommand startblink("+blink", IN_BlinkDown);
static ConCommand stopblink("-blink", IN_BlinkUp);
