...
static ConVar eo_camera_flashlight_fov( "eo_camera_flashlight_fov", "95", FCVAR_CHEAT);
...
void CFlashlightEffect::UpdateLightNew(const Vector &vecPos, const Vector &vecForward, const Vector &vecRight, const Vector &vecUp )
{
	...
	if ( bFlicker == false )
	{
		...
		if(C_BasePlayer::GetLocalPlayer()->IsOverlord() && GetOverlordData() && 
			GetOverlordData()->IsInVehicle())
		{
			state.m_fHorizontalFOVDegrees = eo_camera_flashlight_fov.GetFloat();
			state.m_fVerticalFOVDegrees = eo_camera_flashlight_fov.GetFloat();
		}
		else
		{
