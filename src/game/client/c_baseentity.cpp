...
void C_BaseEntity::RegisterGlow(Color clr, float flGlowScale /*= 1.0f*/)
{
	CEntGlowEffect * pEntGlowEffect = 
				static_cast<CEntGlowEffect*>(g_pScreenSpaceEffects->GetScreenSpaceEffect("ge_entglow"));

	if(pEntGlowEffect)
		pEntGlowEffect->RegisterEnt( EHANDLE(this), clr, flGlowScale );
	else
		Warning("No glow effect!\n");

	m_bGlowRegistered = true;
}

void C_BaseEntity::DeregisterGlow()
{
	CEntGlowEffect * pEntGlowEffect = 
				static_cast<CEntGlowEffect*>(g_pScreenSpaceEffects->GetScreenSpaceEffect("ge_entglow"));

	if(pEntGlowEffect)
		pEntGlowEffect->DeregisterEnt( EHANDLE(this) );
	else
		Warning("No glow effect!\n");

	m_bGlowRegistered = false;
}

void C_BaseEntity::SetGlowColor(Color clr)
{
	CEntGlowEffect * pEntGlowEffect = 
				static_cast<CEntGlowEffect*>(g_pScreenSpaceEffects->GetScreenSpaceEffect("ge_entglow"));

	if(pEntGlowEffect)
		pEntGlowEffect->SetEntColor( EHANDLE(this), clr );
	else
		Warning("No glow effect!\n");
}

Color C_BaseEntity::GetGlowColor()
{
	CEntGlowEffect * pEntGlowEffect = 
				static_cast<CEntGlowEffect*>(g_pScreenSpaceEffects->GetScreenSpaceEffect("ge_entglow"));

	if(pEntGlowEffect)
		return pEntGlowEffect->GetEntColor( EHANDLE(this) );
	else
		Warning("No glow effect!\n");
