...
static ConVar  eo_old_impact_particles("eo_old_impact_particles", "0", FCVAR_CHEAT);
...
void PerformCustomEffects( const Vector &vecOrigin, trace_t &tr, const Vector &shotDir, int iMaterial, int iScale, int nFlags )
{
	...
	// Overlordess: Our own particles here!

	// Cement and wood have dust and flecks
	QAngle angle;
	VectorAngles(tr.plane.normal, angle);
	if (iMaterial == CHAR_TEX_CONCRETE)
	{
		if(eo_old_impact_particles.GetBool())
			FX_DebrisFlecks( vecOrigin, &tr, iMaterial, iScale, bNoFlecks );
		else
			DispatchParticleEffect("ConcreteImpact", vecOrigin - tr.plane.normal, angle);
	}
	else if(iMaterial == CHAR_TEX_TILE) 
	{	
		if(eo_old_impact_particles.GetBool())
			FX_DebrisFlecks( vecOrigin, &tr, iMaterial, iScale, bNoFlecks );
		else
			DispatchParticleEffect("TileImpact", vecOrigin - tr.plane.normal, angle);
	}
	else if(iMaterial == CHAR_TEX_GLASS)
	{
		if(!eo_old_impact_particles.GetBool())
			DispatchParticleEffect("GlassImpact", vecOrigin - tr.plane.normal, angle);
	}
	else if ( iMaterial == CHAR_TEX_WOOD )
	{
		if(eo_old_impact_particles.GetBool())
			...
			DispatchParticleEffect("WoodImpact", vecOrigin - tr.plane.normal, angle);
	}
