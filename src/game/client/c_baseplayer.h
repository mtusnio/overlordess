...
#define OVERLORD_GLOW_COLOR Color(0, 255, 0, 255)
...
public:
	...
	// Overrides health client-side, still keeps the same one on the server (AND CLIENT)
	// only works for displays
	virtual void    SetHealthOverride(int health) { m_iOverrideHealth = health; };
	virtual bool    ShouldOverrideHealth() const { return m_iOverrideHealth > 0; }; 
	virtual int     GetHealthOverride() const { return m_iOverrideHealth; };
	...
	virtual Color				GetPlayerGlowColor();
	...
	void		   EnableProtection(int type) { m_iProtection |= type; };
	void		   DisableProtection(int type) { m_iProtection &= ~type; };
	void		   SetProtection(int type) { m_iProtection = type; };

	int			   GetProtection() const { return m_iProtection; };
	virtual int	   GetDeathsThisRound() const { return m_iRoundDeaths; };
	virtual int    GetPowerPerKill() const;				

	int			   DamageToOverlord() const { return m_DamageToOverlord; };
	bool		   WantsToBeOverlord() const { return m_bWantsOverlord; };

	bool		   CanSpawn() const { return m_bCanSpawn; };
protected:
	...
	PlayerClass_t		    m_PlayerClass;
	bool					m_bCanSwitchClass;

	int						m_DamageToOverlord;
	int						m_iProtection;
	int						m_iBuybacks;
	bool					m_bWantsOverlord;
	int						m_iOverrideHealth;
	bool					m_bCanSpawn;
public:
