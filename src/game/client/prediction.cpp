...
//	g_pMoveData->m_vecOldAngles = player->pl.v_angle;
	...
	{	
		if(pData)
		{
			...
			if(pCam)
			{
				...
				if(g_ConsoleManager.GetBuildMode() > BL_SELECT /*&& !g_ConsoleManager.IsInMouseRotation()*/)
				{
					if(g_ConsoleManager.GetBuildMode() == BL_ROTATE)
						g_ConsoleManager.RotateTrap(ucmd->mousedx, ucmd->mousedy);
					else if(g_ConsoleManager.GetBuildMode() == BL_MOVE)
						g_ConsoleManager.MoveTrap();
				}

				pCam->ProcessMovement(ucmd);
			}
		}
	}
	...
	// RUN MOVEMENT
	// Also make sure we are not in a camera
	if ( !pVehicle )
	{
