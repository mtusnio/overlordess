...
IMPLEMENT_CLIENTCLASS_DT_NOBASE(C_PlayerResource, DT_PlayerResource, CPlayerResource)
	...
	RecvPropArray3( RECVINFO_ARRAY(m_Class), RecvPropInt( RECVINFO(m_Class[0]))),
	RecvPropArray3( RECVINFO_ARRAY(m_bWantsOverlord), RecvPropBool( RECVINFO(m_bWantsOverlord[0]))),
END_RECV_TABLE()
...
BEGIN_PREDICTION_DATA( C_PlayerResource )
	...
	DEFINE_PRED_ARRAY( m_Class, FIELD_INTEGER, MAX_PLAYERS+1, FTYPEDESC_PRIVATE),
	DEFINE_PRED_ARRAY( m_bWantsOverlord, FIELD_BOOLEAN, MAX_PLAYERS+1, FTYPEDESC_PRIVATE),
...
//	memset( m_iPacketloss, 0, sizeof( m_iPacketloss ) );
	...
	m_Colors[TEAM_OVERLORD] = COLOR_GREY;
...
bool C_PlayerResource::IsAlive(int iIndex ) const
{
...
PlayerClass_t C_PlayerResource::GetPlayerClass(int index)
{
	if(!IsConnected(index))
		return 0;
...
const char * C_PlayerResource::GetPlayerClassName(int index)
{
	if(!IsConnected(index))
		return "Unknown";

	//if(GetTeam(index) == TEAM_OVERLORD)
	//	return "Overlordess";
