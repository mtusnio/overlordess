...
// -------------------------------------------------------------------------------- //
	...
	BEGIN_RECV_TABLE_NOBASE( C_BasePlayer, DT_LocalPlayerExclusive )
		...
		RecvPropFloat		( RECVINFO( m_SpawnTime) ),

		RecvPropInt			( RECVINFO( m_nWaterLevel ) ),
		RecvPropFloat		( RECVINFO( m_flLaggedMovementValue )),

		RecvPropBool		( RECVINFO( m_bDrawLocal)),
		RecvPropBool		( RECVINFO( m_bCanSwitchClass)),
		RecvPropInt			( RECVINFO( m_iBuybacks)),
		RecvPropBool		( RECVINFO( m_bWantsOverlord)),
		RecvPropBool		( RECVINFO( m_bCanSpawn)),
	END_RECV_TABLE()
...
// -------------------------------------------------------------------------------- //
	IMPLEMENT_CLIENTCLASS_DT(C_BasePlayer, DT_BasePlayer, CBasePlayer)
		...
		RecvPropInt(RECVINFO(m_bInvisible)),
		RecvPropInt(RECVINFO(m_DamageToOverlord)),
		RecvPropInt(RECVINFO(m_iProtection)),
		RecvPropInt(RECVINFO(m_PlayerClass)),
		RecvPropInt(RECVINFO( m_iRoundDeaths)),
...
BEGIN_PREDICTION_DATA( C_BasePlayer )
	...
	//DEFINE_PRED_FIELD(m_iProtection, FIELD_INTEGER, FTYPEDESC_INSENDTABLE),
...
Color C_BasePlayer::GetPlayerGlowColor()
{
	if(IsRebel())
	{
		int toFull = 70 - (GetMaxHealth() - GetHealth());
		int toEmpty = 80 - GetHealth();
		...
		float toFullFraction =  ((float)toFull)/70.0f;
		float toEmptyFraction = ((float)toEmpty)/80.0f;

		return Color(255.0f * toEmptyFraction, 255.0f * toFullFraction, 0);
	}
	else if(!IsOverlord())
		return Color(255, 0, 0, 255);

	return OVERLORD_GLOW_COLOR;
}
...
bool C_BasePlayer::ShouldDrawLocalPlayer()
{
	return input->CAM_IsThirdPerson() || ( ToolsEnabled() && ToolFramework_IsThirdPersonCamera() ) 
		|| (/*GetLocalPlayer()->IsOverlord() &&*/ GetLocalPlayer()->m_bDrawLocal);
}
