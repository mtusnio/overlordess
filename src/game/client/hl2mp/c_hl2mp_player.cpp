...
#define FLASHLIGHT_DISTANCE_CAMERA 3000
	...
	m_iv_angEyeAngles( "C_HL2MP_Player::m_iv_angEyeAngles" )
{
	...
	m_OverlayType = OVERLAY;
	m_bOverlayOn = false;
	Q_strncpy(m_OverlayMaterial, "\0", ARRAYSIZE(m_OverlayMaterial));
}
...
int C_HL2MP_Player::DrawModel( int flags )
{
	...
	if(m_bOverlayOn)
	{
		if(m_OverlayType == OVERLAY)
		{
			int ret = BaseClass::DrawModel(flags);

		    modelrender->ForcedMaterialOverride( materials->FindMaterial( m_OverlayMaterial, TEXTURE_GROUP_OTHER ) );
			BaseClass::DrawModel(STUDIO_RENDER|STUDIO_TRANSPARENCY);
			modelrender->ForcedMaterialOverride(0);

			return ret;
		}
		else if(m_OverlayType == OVERRIDE)
		{
			modelrender->ForcedMaterialOverride( materials->FindMaterial( m_OverlayMaterial, TEXTURE_GROUP_OTHER ) );
			int ret = BaseClass::DrawModel(STUDIO_RENDER|STUDIO_TRANSPARENCY);
			modelrender->ForcedMaterialOverride(0);
...
void C_HL2MP_Player::UpdateFlashlight()
{
	...
	if ( IsEffectActive( EF_DIMLIGHT ) )
	{
		...
		float distance = FLASHLIGHT_DISTANCE;
		if(!IsOverlord() || (IsOverlord() && (!pData || !pData->IsInVehicle())))
		{
		...
		else if(IsOverlord() && pData && pData->IsInVehicle())
		{
			distance = FLASHLIGHT_DISTANCE_CAMERA;
			QAngle ang;
			VectorAngles(vecForward, ang);
			AngleVectors(ang, &vecForward, &vecRight, &vecUp);
		}


		// Update the light with the new position and direction.		
		m_pHL2MPFlashLightEffect->UpdateLight( position, vecForward, vecRight, vecUp, distance );
	}
...
{
	// We disable sprint
	return m_bSprintEnabled && ( (!m_Local.m_bDucked && !m_Local.m_bDucking) && (GetWaterLevel() != 3) ) 
		&& !(IsOverlord() && (GET_OVERLORD_DATA)->IsInVehicle()) /*&& (MaxSpeed() >= HL2_NORM_SPEED)*/;
...
void C_HL2MP_Player::StartSprinting( void )
{
	...
	// Just add our speed increase now and let the server
	// update us properly server-side
	SetMaxSpeed( MaxSpeed() + (HL2_SPRINT_SPEED - HL2_NORM_SPEED) );
	m_fIsSprinting = true;
}
...
{
	// Same as above, try to calculate
	SetMaxSpeed( MaxSpeed() - (HL2_SPRINT_SPEED - HL2_NORM_SPEED) );
...
void C_HL2MP_Player::SetMaterialOverlay(const char *material, OverlayType_t type, bool bOn)
{
	if(material)
	{
		Q_strncpy(m_OverlayMaterial, material, ARRAYSIZE(m_OverlayMaterial));
...
void C_HL2MPRagdoll::CreateHL2MPRagdoll( void )
{
	...
	// Create particle effects on it
	if(Q_stricmp(m_particleName, "\0"))
	{
		ParticleProp()->Create(m_particleName, PATTACH_ABSORIGIN_FOLLOW);
	}
}
