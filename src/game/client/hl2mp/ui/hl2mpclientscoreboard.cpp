...
void CHL2MPClientScoreBoardDialog::Reset()
{
	...
	InitPlayerList( m_pPlayerListC, TEAM_OVERLORD );
}
...
void CHL2MPClientScoreBoardDialog::UpdateTeamInfo()
{
	...
	for ( int teamIndex = startTeam; teamIndex <= TEAM_REBELS; teamIndex++ )
	{
		...
		if ( team )
		{
			...
			SetDialogVariable( pDialogVarTeamScore, "" );	
...
void CHL2MPClientScoreBoardDialog::UpdatePlayerList()
{
	...
	for( int playerIndex = 1 ; playerIndex <= MAX_PLAYERS; playerIndex++ )
	{
		if( g_PR->IsConnected( playerIndex ) )
		{
			...
			Color clr = GetPlayerColor( playerIndex );
...
void CHL2MPClientScoreBoardDialog::UpdateSpectatorList()
{
	...
	if ( nSpectators > 0 )
	{
		const char *pchFormat = "#ScoreBoard_Spectators";

		wchar_t wzSpectatorCount[16];
		wchar_t wzSpectatorList[512];
...
Color CHL2MPClientScoreBoardDialog::GetPlayerColor(int playerIndex) const
{
	int team = g_PR->GetTeam(playerIndex);
	Color clr = g_PR->GetTeamColor(team);

	if(!g_PR->WantsToBeOverlord(playerIndex) && (team == TEAM_REBELS))
	{
		clr.SetColor(clr.r(), clr.g() + 65, clr.b() + 65, clr.a());
	}
