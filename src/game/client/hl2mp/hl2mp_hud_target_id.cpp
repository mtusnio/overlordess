...
#define MAX_ID_STRING 256
	...
	// Can't see while in console
	GET_OVERLORD_DATA_ASSERT(pData);
	if(pData->IsInVehicle() && pPlayer->IsOverlord())
		return;
	...
	if ( iEntIndex )
	{
		...
		wchar_t wszClassName[20];
		bool bShowHealth = true;
		bool bShowPlayerName = false;
		bool bShowPlayerClass = false;
		wchar_t wszTextDisplay[MAX_AREA_NAME_SIZE];
		...
		if ( IsPlayerIndex( iEntIndex ) )
		{
			...
			if(bShowPlayerClass)
			{
				g_pVGuiLocalize->ConvertANSIToUnicode(pPlayer->GetPlayerClassName(), wszClassName, sizeof(wszClassName));
			}
		}
		else if(cl_entitylist->GetEnt(iEntIndex))
		{
			bShowHealth = false;

			c = GetColorForTargetTeam( pPlayer->GetTeamNumber() );

			printFormatString = cl_entitylist->GetEnt(iEntIndex)->GetTargetTextFormat();

			g_pVGuiLocalize->ConvertANSIToUnicode(cl_entitylist->GetEnt(iEntIndex)->GetTargetText(), wszTextDisplay, sizeof(wszTextDisplay));
				
		}

		if ( printFormatString )
		{
			if ( bShowPlayerName && bShowHealth && bShowPlayerClass )
			{
				g_pVGuiLocalize->ConstructString( sIDString, sizeof(sIDString), g_pVGuiLocalize->Find(printFormatString), 3, wszClassName, wszPlayerName, wszHealthText );
			}
			...
			{
				g_pVGuiLocalize->ConstructString( sIDString, sizeof(sIDString), g_pVGuiLocalize->Find(printFormatString), 1, wszTextDisplay );
			}
		}
