...
public:
	...
	// Spares us the use of dynamic_cast for weapons
	// in Overlord mod
	virtual bool			IsClassWeapon(void) const { return true; };
	
	virtual bool		   IsOverlordWeapon() const { return false; };
	virtual PlayerClass_t  GetClassWeapon() const { return CLASS_ALL; }; 

	virtual const char * GetPrimaryMode() const { return "None"; }; 
	virtual const char * GetSecondaryMode() const { return "None"; }; 
	virtual const char * GetCostLabel() const { return ""; };
public:
	...
	virtual bool			UsesAmmo( void ) const { return true; };
...
public:
	...
	CNetworkVar( int, m_iMaxClip1);
	CNetworkVar( int, m_iMaxClip2);
