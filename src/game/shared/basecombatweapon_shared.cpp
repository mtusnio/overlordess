...
void CBaseCombatWeapon::Drop( const Vector &vecVelocity )
{
	...
	// Class weapons shouldn't be dropped
	if(IsClassWeapon())
	{
		UTIL_Remove(this);
		return;
...
void CBaseCombatWeapon::OnPickedUp( CBaseCombatCharacter *pNewOwner )
{
	...
	if( pNewOwner->IsPlayer() )
	{
		...
		if(pPlayer)
		{
			// DO NOT LET US PICK OV WEAPONS UP!
			...
			// Do not let us pick up non-class weapons
			if(IsClassWeapon() && GetClassWeapon() != CLASS_ALL)
			{
				if((pPlayer->GetPlayerClass() != GetClassWeapon()) || pPlayer->IsOverlord())
				{
					UTIL_Remove(this);
					return;
				}
...
void CBaseCombatWeapon::ItemPostFrame( void )
{
	...
	if(pOwner->IsOverlord() && GetOverlordData()->IsInVehicle())
		return;
...
BEGIN_NETWORK_TABLE_NOBASE( CBaseCombatWeapon, DT_LocalWeaponData )
	...
	SendPropIntWithMinusOneFlag( SENDINFO(m_iMaxClip1), 8),
	SendPropIntWithMinusOneFlag( SENDINFO(m_iMaxClip2), 8),
	...
	RecvPropIntWithMinusOneFlag( RECVINFO( m_iMaxClip1 ) ),
	RecvPropIntWithMinusOneFlag( RECVINFO( m_iMaxClip2 ) ),
