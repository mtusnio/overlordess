...
bool CMultiplayRules::Init()
{
	...
	float CMultiplayRules::FlPlayerFallDamage( CBasePlayer *pPlayer )
	{
		...
		case 2: // percental
			...
			return pPlayer->GetMaxHealth() * (pPlayer->m_Local.m_flFallVelocity * DAMAGE_FOR_FALL_SPEED)/100;
			break;
		case 1://progressive
			pPlayer->m_Local.m_flFallVelocity -= PLAYER_MAX_SAFE_FALL_SPEED;
	...
	CBasePlayer *CMultiplayRules::GetDeathScorer( CBaseEntity *pKiller, CBaseEntity *pInflictor )
	{
		if ( pKiller)
		{
			if ( (pKiller->Classify() == CLASS_PLAYER) || (pKiller->Classify() == CLASS_OVERLORD) )
