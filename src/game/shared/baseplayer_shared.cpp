...
ConVar hl2_normspeed( "hl2_normspeed", "185" );
ConVar hl2_sprintspeed( "hl2_sprintspeed", "235" );

ConVar eo_playtest_overlordhealth(
	"eo_playtest_overlordhealth", 
	"700", 
	FCVAR_REPLICATED | 
...
ConVar eo_playtest_assaulthealth(
	"eo_playtest_assaulthealth", 
	"100", 
	FCVAR_REPLICATED | 
#ifndef OVER_PLAYTEST
	FCVAR_HIDDEN |
...
ConVar eo_playtest_psionhealth(
	"eo_playtest_psionhealth", 
	"100", 
	FCVAR_REPLICATED | 
#ifndef OVER_PLAYTEST
	FCVAR_HIDDEN |
...
ConVar eo_playtest_hackerhealth(
	"eo_playtest_hackerhealth", 
	"100", 
	FCVAR_REPLICATED | 
#ifndef OVER_PLAYTEST
	FCVAR_HIDDEN |
...
ConVar eo_playtest_stealthhealth(
	"eo_playtest_stealthhealth", 
	"100", 
	FCVAR_REPLICATED | 
#ifndef OVER_PLAYTEST
	FCVAR_HIDDEN |
...
ConVar eo_playtest_convertedhealth(
	"eo_playtest_convertedhealth", 
	"300", 
	FCVAR_REPLICATED | 
#ifndef OVER_PLAYTEST
	FCVAR_HIDDEN |
...
ConVar eo_playtest_tankarmour(
	"eo_playtest_tankarmour", 
	"70", 
	FCVAR_REPLICATED | 
#ifndef OVER_PLAYTEST
	FCVAR_HIDDEN |
...
ConVar eo_playtest_phasingarmour(
	"eo_playtest_phasingarmour", 
	"30", 
	FCVAR_REPLICATED | 
#ifndef OVER_PLAYTEST
	FCVAR_HIDDEN |
...
ConVar eo_playtest_techarmour(
	"eo_playtest_techarmour", 
	"50", 
	FCVAR_REPLICATED | 
#ifndef OVER_PLAYTEST
	FCVAR_HIDDEN |
...
ConVar eo_playtest_assaultarmour(
	"eo_playtest_assaultarmour", 
	"50", 
	FCVAR_REPLICATED | 
#ifndef OVER_PLAYTEST
	FCVAR_HIDDEN |
...
ConVar eo_playtest_psionarmour(
	"eo_playtest_psionarmour", 
	"50", 
	FCVAR_REPLICATED | 
#ifndef OVER_PLAYTEST
	FCVAR_HIDDEN |
...
ConVar eo_playtest_hackerarmour(
	"eo_playtest_hackerarmour", 
	"40", 
	FCVAR_REPLICATED | 
#ifndef OVER_PLAYTEST
	FCVAR_HIDDEN |
...
ConVar eo_playtest_stealtharmour(
	"eo_playtest_stealtharmour", 
	"30", 
	FCVAR_REPLICATED | 
#ifndef OVER_PLAYTEST
	FCVAR_HIDDEN |
...
ConVar eo_base_power_per_kill("eo_base_power_per_kill", "85", FCVAR_REPLICATED | FCVAR_CHEAT);
ConVar eo_max_bonus_power("eo_max_bonus_power", "40", FCVAR_REPLICATED | FCVAR_CHEAT);
ConVar eo_first_kill_power("eo_first_kill_power", "10", FCVAR_REPLICATED | FCVAR_CHEAT);
...
void CBasePlayer::ItemPostFrame()
{
	...
	// Currently here we are going to decide which player should have glow enabled
	// and who shouldn't.
	// OPTIMISE!
	for(int i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		if(IsOverlord() && !GetOverlordData()->IsWallSightDisabled())
		{
			if(pPlayer->IsGlowRegistered())
			{
				if(pPlayer->IsPlayerDead() || (pPlayer->GetTeamNumber() == TEAM_SPECTATOR) ||
					(pPlayer->GetEffects() & EF_NODRAW))
					pPlayer->DeregisterGlow();

				pPlayer->SetGlowColor(pPlayer->GetPlayerGlowColor());
			}
			else
			{
				if(pPlayer->IsAlive() && pPlayer->IsRebel() && !(pPlayer->GetEffects() & EF_NODRAW))
				{
					pPlayer->RegisterGlow(pPlayer->GetPlayerGlowColor());
				}
			}
		}
		else
		{
			if(pPlayer->IsOverlord() && !pPlayer->IsInvisible() && 
				pPlayer->IsAlive() && GetOverlordData()->IsOverlordMarked())
			{
				if(!pPlayer->IsGlowRegistered())
				{
					pPlayer->RegisterGlow(pPlayer->GetPlayerGlowColor());
			...
			else if(pPlayer->IsGlowRegistered())
				pPlayer->DeregisterGlow();
		}
...
const QAngle &CBasePlayer::EyeAngles( )
{
	...
	if(IsOverlord() && GET_OVERLORD_DATA && (GET_OVERLORD_DATA)->IsInVehicle())
	{
		angEyeWorld = (GET_OVERLORD_DATA)->GetVehicleAngle();
		return angEyeWorld;
	}
	...
	if(pWeapon)
	{
		...
		if(!Q_stricmp(pWeapon->GetClassname(), "weapon_teleport"))
		{
			angle = static_cast<COverlordTeleport*>(pWeapon)->GetCameraAngles();;
		}
		else if(!Q_stricmp(pWeapon->GetClassname(), "weapon_camera"))
		{
			angle = static_cast<COverlordCamWeap*>(pWeapon)->GetCameraAngles();;
		}
...
QAngle CBasePlayer::RealEyeAngles()
{
	// Another copy-pasta...
...
{
	if(IsOverlord() && GET_OVERLORD_DATA && (GET_OVERLORD_DATA)->IsInVehicle())
		return (GET_OVERLORD_DATA)->GetVehicleVector();
	...
	if(pWeapon)
	{
		...
		if(!Q_stricmp(pWeapon->GetClassname(), "weapon_teleport"))
		{
			origin = static_cast<COverlordTeleport*>(pWeapon)->GetCameraOrigin();
		}
		else if(!Q_stricmp(pWeapon->GetClassname(), "weapon_camera"))
		{
			origin = static_cast<COverlordCamWeap*>(pWeapon)->GetCameraOrigin();
		}
	...
	// For weapons overriding camera origin,
	// less rigid implementation needed
	if ( GetVehicle() != NULL )
	{
...
Vector CBasePlayer::RealEyePosition()
{
	// For weapons overriding camera origin,
	// less rigid implementation needed
	if ( GetVehicle() != NULL )
	...
	if ( !pVehicle )
	{
		...
		else
		{
			/*(if(IsOverlord())
			{
				...
				{
					CalcOverlordView( eyeOrigin, eyeAngles, fov );
				}
				else
				{
...
void CBasePlayer::CalcOverlordView(Vector &eyeOrigin, QAngle &eyeAngles, float &fov)
{
	#if defined( CLIENT_DLL )
	...
	VectorCopy(pData->GetVehicleVector(), eyeOrigin);

	VectorCopy(pData->GetVehicleAngle(), eyeAngles );
...
bool CBasePlayer::IsOverlord() const
{
...
bool CBasePlayer::IsRebel() const
{
...
bool CBasePlayer::IsConverted() const
{
	if(GetTeamNumber() == TEAM_CONVERTED)
		return true;

	return false;
}

// Returns true if the player is basically a rebel who hasn't been thrown out of the round. 
// Kept for compatibility with older code
bool CBasePlayer::IsPlayingRebel() const
{
	if(!IsRebel())
		return false;
...
PlayerClass_t CBasePlayer::GetPlayerClass(void) const
{ 
	return m_PlayerClass; 
}

const char * CBasePlayer::GetPlayerClassName() const
{
	// Little hack, lets us get the array index for the class
	int i = 0;
	bool bFound = false;
	for(; i < CLASS_NUMBER; i++)
	{
		if((1 << i) == GetPlayerClass())
		{
			bFound = true;
			break;
		}
	...
	return g_PlayerClassnames[i]; 
}

bool CBasePlayer::IsInvisible() const
{
	return m_bInvisible != INVIS_OFF;
}


int CBasePlayer::GetPowerPerKill() const
{
	int initial = HL2MPRules()->GetInitialPlayers();

	if(initial == 0)
	{
		Warning("No initial players, no power per kill!\n");
		return 0;
	}

	int basePower = eo_base_power_per_kill.GetInt() / initial;
	int deaths = HL2MPRules()->GetDeathsAmount();

	if(deaths == 0)
		return basePower + eo_first_kill_power.GetInt();

	float percentage = 1.0f - ((float)GetDeathsThisRound() / (float)deaths);
	int bonus = eo_max_bonus_power.GetFloat() * percentage;
