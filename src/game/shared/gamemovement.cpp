...
ConVar eo_wall_force_factor("eo_wall_force_factor", "1.31", FCVAR_REPLICATED | FCVAR_CHEAT);
ConVar eo_wall_hit_interval("eo_wall_hit_interval", "0.5", FCVAR_REPLICATED | FCVAR_CHEAT);
ConVar eo_wall_hit_tolerance("eo_wall_hit_tolerance", "80", FCVAR_REPLICATED | FCVAR_CHEAT);

ConVar eo_overlord_jump("eo_overlord_jump", "520", FCVAR_REPLICATED | FCVAR_CHEAT);
...
bool CGameMovement::CheckJumpButton( void )
{
	...
	// Don't jump if in console
	COverlordData * pData = GetOverlordData();
	if(pData && player->IsOverlord() && pData->IsInVehicle())
		return false;
	...
	if ( g_bMovementOptimizations )
	{
		...
		else
		{
			...
			{
				flMul = eo_overlord_jump.GetFloat();		
			}
			else
			{
				flMul = 160.0f;	// approx. 21 units.
...
int CGameMovement::TryPlayerMove( Vector *pFirstDest, trace_t *pFirstTrace )
{
	...
	float maxVelocity = PLAYER_FATAL_FALL_SPEED * eo_wall_force_factor.GetFloat();
	float minVelocity = PLAYER_MAX_SAFE_FALL_SPEED * eo_wall_force_factor.GetFloat();
	if ( fLateralStoppingAmount >= maxVelocity  )
	{
		fSlamVol = 1.0f;
	}
	else if ( fLateralStoppingAmount > minVelocity )
	{
		//fSlamVol = 0.85f;
		fSlamVol = (fLateralStoppingAmount - minVelocity)/(maxVelocity - minVelocity);
	}
#ifndef CLIENT_DLL
	/*// Take wall damage
	if(fSlamVol > 0.0f)
	{
		...
		// Don't let the player receive damage indifinetely from one wall hit
		// and make sure he moved before adding more damage
		if(player->GetWallDamageInterval() <= gpGlobals->curtime)
		{
			float damage = player->GetMaxHealth() * fSlamVol;
			player->TakeDamage( CTakeDamageInfo( GetContainingEntity(INDEXENT(0)), GetContainingEntity(INDEXENT(0)), 
				damage, DMG_FALL ) ); 
			player->SetWallDamageInterval(gpGlobals->curtime + eo_wall_hit_interval.GetFloat());
			player->SetLastWallHitPosition(player->GetAbsOrigin());
			MoveHelper()->StartSound( player->GetAbsOrigin(), "Player.FallDamage" );
		}
		else
		{
			// Delay next hit if we are stuck, prevents 
			// us from getting hit over and over again
			Vector pos = player->GetAbsOrigin();
			if(((pos - player->GetLastWallHitPosition()).Length()) <= eo_wall_hit_tolerance.GetFloat())
			{
				player->SetWallDamageInterval(gpGlobals->curtime + eo_wall_hit_interval.GetFloat());
				player->SetLastWallHitPosition(player->GetAbsOrigin());
			}
...
void CGameMovement::CheckFalling( void )
{
		 ...
		 player->m_Local.m_flFallVelocity >= PLAYER_FALL_PUNCH_THRESHOLD )
	{
		...
		else if(player->IsOverlord())
		{
			// Overlordess takes no fall damage
		}
		else
		{
