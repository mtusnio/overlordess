...
ConVar eo_player_ratio_restart("eo_player_ratio_restart", "3.5", FCVAR_REPLICATED | FCVAR_CHEAT,
							   "If there is this amount of times more players than initial players, auto-restart the game. 0 disables");
...
//#define STARTING_POWER 50


#endif
#include "tier0/vprof.h"

//ConVar eo_health_per_kill("eo_health_per_kill", "25", FCVAR_REPLICATED | FCVAR_CHEAT);
...
BEGIN_NETWORK_TABLE_NOBASE( CHL2MPRules, DT_HL2MPRules )
		...
		RecvPropInt( RECVINFO( m_InitialPlayers ) ),
		RecvPropInt( RECVINFO( m_Deaths ) ),
	#else
		SendPropBool( SENDINFO( m_bTeamPlayEnabled ) ),
		SendPropFloat( SENDINFO( m_flGameStartTime ) ),
		SendPropInt( SENDINFO( m_InitialPlayers ) ),
		SendPropInt( SENDINFO( m_Deaths ) ),
	#endif
...
static const char *s_PreserveEnts[] =
{
	...
	"env_wind",
	"env_fog_controller",
	//"func_brush", // Commented, caused the brushes to dissapear
	"func_wall",
	"func_buyzone",
	"func_illusionary",
	"infodecal",
...
ConVar eo_playtest_doublepoints("eo_playtest_doublepoints", "0", FCVAR_REPLICATED
...
CHL2MPRules::CHL2MPRules()
{
	...
	m_flLastRestart = gpGlobals->curtime;
	/*m_bInVehicle = false;
	m_iPower = STARTING_POWER;
	m_vVehicleVector.Init();
	m_VehicleAngle.Init();*/
...
void CHL2MPRules::PlayerKilled( CBasePlayer *pVictim, const CTakeDamageInfo &info )
{
	...
	GetOverlordData()->UpdateFerocity(pVictim);
	...
	// Add power if that's a rebel
	if(pVictim && pVictim->IsRebel())
	{
		pData->HandlePowerEvent(EVENT_PLAYERKILLED, pVictim->GetPowerPerKill());
		HL2MPRules()->IncrementDeathsAmount();	
	}
...
void CHL2MPRules::OverlordLoses()
{
#ifndef CLIENT_DLL
	mp_restartgame.SetValue(ROUND_RESTART_TIME);
	GET_OVERLORD_DATA->OverlordLoses();
	CheckRestartGame();

	if(eo_playtest_doublepoints.GetBool())
	{
		...
		if(pRebels)
		{
			for(int i = 0; i < pRebels->GetNumPlayers(); i++)
			{
				...
				if(pPlayer->IsAlive())
				{
					pPlayer->IncreaseOverlordDamage(pPlayer->DamageToOverlord());
				}
			}
		}
	}

	// Notify about the next Ov!
	CBasePlayer * pNewOverlord = (GET_OVERLORD_DATA)->GetNextOverlord();

	if(pNewOverlord)
	{
		char text[256];
		Q_snprintf(text, ARRAYSIZE(text), "Overlordess is dead! %s will take her place!\n", pNewOverlord->GetPlayerName());
		
		// Override the next Ov
		GET_OVERLORD_DATA->SetNextOverlord(pNewOverlord);

		UTIL_ClientPrintAll(HUD_PRINTTALK, text);
	}
	else
	{
		UTIL_ClientPrintAll(HUD_PRINTTALK, "Overlordess is dead!");
	}

#endif
}

void CHL2MPRules::OverlordWins()
{
#ifndef CLIENT_DLL
	mp_restartgame.SetValue(ROUND_RESTART_TIME);
	UTIL_ClientPrintAll(HUD_PRINTTALK, "Overlordess has won!");
	GET_OVERLORD_DATA->OverlordWins();
	CheckRestartGame();
#endif
}
...
//	const float flFragLimit = 0.0f; 
	...
	CTeam *pCombine = g_Teams[TEAM_OVERLORD];
	CTeam *pRebels = g_Teams[TEAM_REBELS];
	//CTeam *pConverted = g_Teams[TEAM_CONVERTED];
	CTeam *pSpectator = g_Teams[TEAM_SPECTATOR];
	...
	if(mp_restartgame.GetInt() == 0 && m_flRestartGameTime == 0.0f)
	{
		// All players dead, captured or converted? Restart.
		//CTeam *pRebels = GetGlobalTeam(TEAM_REBELS);
		//CTeam *pCombine = GetGlobalTeam(TEAM_CONVERTED);

		//if(pRebels && ( pRebels->GetNumPlayers() > 0 ) )
		if(pRebels && pSpectator && (pSpectator->GetNumPlayers() > 0) && ( pRebels->GetNumPlayers() <= 0 ) 
			&& (GetOverlordData()->GetSpawnsAmount() < 1) )
		{
			OverlordWins();
		}
	}
	...
	if(mp_restartgame.GetInt() == 0 
		&& HL2MPRules()->m_flRestartGameTime == 0.0f)
	{
		if(!GetOverlordData()->IsOverlordPlaying())
		{
			mp_restartgame.SetValue(5);

			UTIL_ClientPrintAll(HUD_PRINTCENTER,"The Overlordess has retreated!");
		}

		if((GetInitialPlayers() != 0) && (eo_player_ratio_restart.GetInt() != 0))
		{
			CTeam * pRebels = GetGlobalTeam(TEAM_REBELS);
			CTeam * pSpectators = GetGlobalTeam(TEAM_SPECTATOR);

			if(pRebels && pSpectators)
			{
				int players = pRebels->GetNumPlayers() + pSpectators->GetNumPlayers();

				if(players >= (eo_player_ratio_restart.GetFloat() * (float)GetInitialPlayers()))
				{
					mp_restartgame.SetValue(5);

					GetOverlordData()->SetNextOverlord(GetOverlordData()->GetOverlord());
					UTIL_ClientPrintAll(HUD_PRINTCENTER, "Restarting the game due to player disbalance...");
				}
			}
		}
	}
...
void CHL2MPRules::DeathNotice( CBasePlayer *pVictim, const CTakeDamageInfo &info )
{
	...
	else
	{
		...
		{
			// Checks the pointer, caused some strange crash
			if(pInflictor)
		...
		else if ( strcmp( killer_weapon_name, "grenade_emp" ) == 0 )
		{
...
void CHL2MPRules::ClientSettingsChanged( CBasePlayer *pPlayer )
{
	...
	if ( stricmp( szModelName, pCurrentModel ) )
	{
		...
		/*else
		{
			...
			{
				pHL2Player->ChangeTeam( TEAM_OVERLORD );
			}
		}*/
	}
...
#ifndef CLIENT_DLL
// Stolen from hl2_gamerules
void CHL2MPRules::InitDefaultAIRelationships()
{
	int i, j;
	...
	// Converted
	CBaseCombatCharacter::SetDefaultRelationship(CLASS_CONVERTED,		CLASS_PLAYER,			D_HT, 0);
	CBaseCombatCharacter::SetDefaultRelationship(CLASS_CONVERTED,		CLASS_OVERLORD,			D_LI, 0);
}
#endif
...
CAmmoDef *GetAmmoDef()
{
	...
	if ( !bInitted )
	{
		...
		def.AddAmmoType("SMG1",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	0,			0,			110,		BULLET_IMPULSE(200, 1225),	0 );
		def.AddAmmoType("357",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	0,			0,			30,			BULLET_IMPULSE(800, 5000),	0 );
		//def.AddAmmoType("XBowBolt",			DMG_BULLET,					TRACER_LINE,			0,			0,			10,			BULLET_IMPULSE(800, 8000),	0 );
		def.AddAmmoType("Buckshot",			DMG_BULLET | DMG_BUCKSHOT,	TRACER_LINE,			0,			0,			20,			BULLET_IMPULSE(400, 1200),	0 );
		def.AddAmmoType("RPG_Round",		DMG_BURN,					TRACER_NONE,			0,			0,			3,			0,							0 );
		//def.AddAmmoType("SMG1_Grenade",		DMG_BURN,					TRACER_NONE,			0,			0,			3,			0,							0 );
		def.AddAmmoType("EMPGrenade",		DMG_BURN,				    TRACER_NONE,			0,			0,			2,			0,							0 );
		def.AddAmmoType("Grenade",			DMG_BURN,					TRACER_NONE,			0,			0,			3,			0,							0 );
		def.AddAmmoType("Grenadesmoke",		DMG_GENERIC,				TRACER_NONE,			0,			0,			2,			0,							0 );
		def.AddAmmoType("Marker",			DMG_GENERIC,				TRACER_NONE,			0,			0,			0,			0,							0 );
		def.AddAmmoType("slam",				DMG_BURN,					TRACER_NONE,			0,			0,			2,			0,							0 );
		
		def.AddAmmoType("Charge",			DMG_GENERIC,				TRACER_NONE,			0,			0,			0,			0,							0 );		

		def.AddAmmoType("Rifle",			DMG_BULLET,				    TRACER_LINE_AND_WHIZ,   0,          0,          255,        BULLET_IMPULSE(300, 1300),  0 );
		def.AddAmmoType("Disabler",			DMG_BULLET,					TRACER_LINE_AND_WHIZ,   0,			0,			145,		BULLET_IMPULSE(200, 900),   0);
		def.AddAmmoType("SniperRound",	    DMG_BULLET,				    TRACER_LINE_AND_WHIZ,   0,          0,          90,       BULLET_IMPULSE(300, 1300),  0 );
	}
...
float CHL2MPRules::GetRoundTime() const
{ 
	return (gpGlobals->curtime - m_flLastRestart); 
}
...
void CHL2MPRules::RestartGame()
{
	...
	// Stop the powerups
	GetOverlordData()->ResetPowerups();

	CBasePlayer * pNewOverlord = NULL;
	CHL2MP_Player *pPlayer = NULL;

	// Select next overlord
	pNewOverlord = GET_OVERLORD_DATA->GetNextOverlord();

	// This thingy is obsolete, it will only run if there is no current overlord. 
	// Should the Ov win she remains for the next round
	if(!pNewOverlord)
	{
		// Maximum number of iterations, stops the while loop from 
		...
		{
			iterations++;
			randomint = random->RandomInt(1, gpGlobals->maxClients);
			if(UTIL_PlayerByIndex(randomint) != GET_OVERLORD_DATA->GetOverlord())
				pPlayer = ToHL2MPPlayer(UTIL_PlayerByIndex(randomint));
		}
		while(!pPlayer && iterations <= 400);

		// This only happens if, due to some VERY bad luck
		// (or when no players are on the server),
		// the server wasn't able to find any player
		if(!pPlayer)
		{
			for (int i = 1; i <= gpGlobals->maxClients; i++ )
			{
	...
	for (int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		pPlayer = (CHL2MP_Player*) UTIL_PlayerByIndex( i );

		if ( !pPlayer )
			continue;

		if(pPlayer == pNewOverlord)
			continue;
	...
	{
		// Basic reset here...
		((CHL2MP_Player*)pNewOverlord)->Reset(false);
		GET_OVERLORD_DATA->DispatchNewOverlord(pNewOverlord);
	}

	// Respawn entities (glass, doors, etc..)

//	CTeam *pRebels = GetGlobalTeam( TEAM_REBELS );
//	CTeam *pOverlord = GetGlobalTeam( TEAM_OVERLORD );
//	CTeam *pCombine = GetGlobalTeam( TEAM_CONVERTED );
	...
	// Remove the Ov from the counter
	m_InitialPlayers--;
}
...
void CHL2MPRules::CheckRestartGame( void )
{
	...
	if ( iRestartDelay > 0 )
	{
		...
		UTIL_ClientPrintAll( HUD_PRINTTALK, "Game will restart in %s1 %s2", strRestartDelay, iRestartDelay == 1 ? "SECOND" : "SECONDS" );
		...
		GET_OVERLORD_DATA->DisableWarMode();
	}
...
/*CPointEntity * CHL2MPRules::PickJail()
{
	CBaseEntity * pEnt = gEntList.FindEntityByClassname(gEntList.FirstEnt(), "info_player_jail");

	if(!pEnt)
		return NULL;

	CPointEntity * pJail = dynamic_cast<CPointEntity*>(pEnt);
