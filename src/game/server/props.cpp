...
void CBreakableProp::Break( CBaseEntity *pBreaker, const CTakeDamageInfo &info )
{
	...
	if ( m_explodeDamage > 0 || m_explodeRadius > 0 )
	{
		if( HasInteraction( PROPINTER_PHYSGUN_BREAK_EXPLODE ) )
		{
			ExplosionCreate( WorldSpaceCenter(), angles, NULL, m_explodeDamage, m_explodeRadius, 
...
IMPLEMENT_SERVERCLASS_ST( CPhysicsProp, DT_PhysicsProp )
	SendPropBool( SENDINFO( m_bCanBuildOn ) ),
	SendPropBool( SENDINFO( m_bAwake ) ),
	SendPropBool( SENDINFO( m_bConstrained) ),
END_SEND_TABLE()
...
void CPhysicsProp::Spawn( )
{
	...
	// Decide whether we can build on it
	if( FClassnameIs( this, "prop_physics_buildable" ) )
	{
		m_bCanBuildOn = true;
	}
	else
...
//-----------------------------------------------------------------------------
void CPhysicsProp::CheckForConstrained()
{
	m_bConstrained = true;
...
LINK_ENTITY_TO_CLASS( prop_physics_buildable, CPhysicsPropMultiplayer );
