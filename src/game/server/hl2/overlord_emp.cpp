...
#define EMP_EXPLOSION_SOUND "NPC_RollerMine.Shock"

ConVar eo_emp_disable_time("eo_emp_disable_time", "4.25", FCVAR_REPLICATED | FCVAR_CHEAT);
ConVar eo_emp_radius("eo_emp_radius", "280", FCVAR_REPLICATED | FCVAR_CHEAT);
ConVar eo_emp_sight_disable_length("eo_emp_sight_disable_length", "10.0", FCVAR_REPLICATED | FCVAR_CHEAT);
...
LINK_ENTITY_TO_CLASS( grenade_emp, COverlordEMP );

void COverlordEMP::Spawn( void )
{
	Precache( );
	SetSolid( SOLID_BBOX );
...
//	UTIL_SetSize(this, Vector(0, 0, 0), Vector(0, 0, 0));
	...
	SetUse( &COverlordEMP::DetonateUse );
	SetTouch( &COverlordEMP::GrenadeAR2Touch );
	SetThink( &COverlordEMP::GrenadeAR2Think );
...
// Output :
//-----------------------------------------------------------------------------
void COverlordEMP::GrenadeAR2Think( void )
{
...
void COverlordEMP::Event_Killed( const CTakeDamageInfo &info )
{
	Detonate( );
}

void COverlordEMP::GrenadeAR2Touch( CBaseEntity *pOther )
{
	Assert( pOther );
	if ( !pOther->IsSolid() )
		return;
...
void COverlordEMP::Detonate(void)
{
	if (!m_bIsLive)
	{
		return;
...
/*	CPASFilter filter( GetAbsOrigin() );
	...
	DispatchParticleEffect(EMP_EFFECT, GetAbsOrigin(), QAngle(0, 0, 0));
	CBaseEntity * pList[1024];
	int count = UTIL_EntitiesInSphere(pList, ARRAYSIZE(pList), WorldSpaceCenter(), m_DmgRadius, 0);

	for(int i = 0; i < count; i++)
	{
		COverlordTrap * pTrap = COverlordTrap::EntityToTrap(pList[i]);

		if(pTrap && pTrap->CanDisableTrap())
		{
			pTrap->DisableTrap(gpGlobals->curtime + eo_emp_disable_time.GetFloat());
			continue;
		}
		...
		{
			GetOverlordData()->DisableWallSight(eo_emp_sight_disable_length.GetFloat());
			pPlayer->KillArmour();
		}
	}

	EmitSound(EMP_EXPLOSION_SOUND);
...
void COverlordEMP::Precache( void )
{
	PrecacheParticleSystem(EMP_EFFECT);
	PrecacheScriptSound(EMP_EXPLOSION_SOUND);

	PrecacheModel("models/Weapons/ar2_grenade.mdl"); 
}


COverlordEMP::COverlordEMP(void)
{
	m_hSmokeTrail  = NULL;
}
