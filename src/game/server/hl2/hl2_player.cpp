...
// SUIT POWER DEVICES
//
#define SUITPOWER_CHARGE_RATE	10											// 100 units in 10 seconds

ConVar eo_sprint_drain("eo_sprint_drain", "25.0", FCVAR_REPLICATED | FCVAR_CHEAT);

#ifdef HL2MP
	CSuitPowerDevice SuitDeviceSprint( bits_SUIT_DEVICE_SPRINT, eo_sprint_drain.GetFloat() );				// 100 units in 4 seconds
#else
...
bool CHL2_Player::CanSprint()
{
	...
	return ( m_bSprintEnabled &&										// Only if sprint is enabled 
			...
			!(IsOverlord() && (GET_OVERLORD_DATA)->IsInVehicle()) /*&&	// Ov cannot sprint in camera
...
void CHL2_Player::StartSprinting( void )
{
	...
	//AddSlowdown(HL2_NORM_SPEED - HL2_SPRINT_SPEED, -1.0f, this);
...
//#endif
	...
	// Do not let the stealther use anything besides 
	// traps when invisible
	if(pUseEntity && IsInvisible() && (GetPlayerClass() == CLASS_STEALTH))
	{
		...
		{
			COverlordTrap * pTrap = COverlordTrap::EntityToTrap(pUseEntity);
			if(pTrap)
			{	
