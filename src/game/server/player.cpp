...
ConVar eo_blink_time("eo_blink_time", "3.0", FCVAR_CHEAT | FCVAR_REPLICATED);
ConVar eo_blink_cooldown("eo_blink_cooldown", "300", FCVAR_CHEAT | FCVAR_REPLICATED);
ConVar eo_blink_slowdown("eo_blink_slowdown", "150", FCVAR_CHEAT | FCVAR_REPLICATED);
...
#define ARMOUR_RECHARGE_DELAY 12.0f
#define ARMOUR_RECHARGE_AMOUNT 8.0f
#define ARMOUR_TIME_CAP 9.0f
#define ARMOUR_RECHARGE_TIME 0.1f
...
ConVar	eo_sk_overlord_head( "eo_sk_overlord_head", "1.5");
ConVar	sk_player_chest( "sk_player_chest","1" );
ConVar	sk_player_stomach( "sk_player_stomach","1" );
ConVar	sk_player_arm( "sk_player_arm","0.95" );
ConVar	sk_player_leg( "sk_player_leg","0.95" );
...
static ConCommand eo_exitconsole("eo_exitconsole", CC_EO_ExitConsole, "Exits the console.");

void CC_EO_ActivateModule( const CCommand &args  )
{
	CBasePlayer *pPlayer = UTIL_GetCommandClient();
	...
	int module = atoi(args[1]);

	cam->FireOnModule(module);
		
}
static ConCommand eo_activatemodule("eo_activate_module", CC_EO_ActivateModule, "Activates a module");


/*CON_COMMAND(eo_lasthacked, "Switch to camera near last hacked console")
{
	CBasePlayer * pPlayer = UTIL_GetCommandClient();
	...
	COverlordDoorLock * pLock = COverlordDoorLock::GetLastHacked();

	if(!pLock)
		return;

	// Look for the nearest camera
	COverlordCamera * pFinal = NULL;
	COverlordCamera * pCam = NULL;
	float distance = FLT_MAX;
	while((pCam = 
		static_cast<COverlordCamera*>(gEntList.FindEntityByClassname(pCam, "point_overcamera"))) != NULL)
	{
		float dist = (pCam->GetAbsOrigin() - pLock->WorldSpaceCenter()).Length();

		if(dist >= distance)
			continue;
		
		if(pCam->GetArea() && pLock->GetAreaName(AREAS-1) && pCam->GetArea()->GetAreaName())
		{
			if(Q_stricmp(pCam->GetArea()->GetAreaName(), pLock->GetAreaName(AREAS-1)))
				continue;
		}
		else if(pCam->GetArea())
			continue;
...
void CC_EO_SelectClass( const CCommand &args  )
{
	CBasePlayer *pPlayer = UTIL_GetCommandClient();
	...
	PlayerClass_t playerclass = CLASS_DEFAULT;

	if(pPlayer->IsRebel())
	{
		if(!Q_strcmp(args[1], "classassault"))
			playerclass = CLASS_ASSAULT;
		/*else if(!Q_strcmp(args[1], "classpsion"))
			playerclass = CLASS_PSION;*/
		else if(!Q_strcmp(args[1], "classhacker"))
			playerclass = CLASS_HACKER;
		else if(!Q_strcmp(args[1], "classstealth"))
			playerclass = CLASS_STEALTH;
		else
		{
			short rand = random->RandomInt(1, 4);
			// Can't select psion now
			playerclass = (1 << rand);
			while(playerclass == CLASS_PSION)
			{
				rand = random->RandomInt(1, CLASS_REBEL_COUNT);
				playerclass = (1 << rand);
			}
		}
	}
	...
	{
		if(!Q_strcmp(args[1], "classtank"))
			playerclass = CLASS_TANK;
		else if(!Q_strcmp(args[1], "classphasing"))
			playerclass = CLASS_PHASING;
		else if(!Q_strcmp(args[1], "classtech"))
			playerclass = CLASS_TECH;
		else
		{
			short rand = random->RandomInt(CLASS_REBEL_COUNT+1, CLASS_REBEL_COUNT+CLASS_OVERLORD_COUNT);
			playerclass = (1 << rand);
		}
	}

	pPlayer->ResolveSelectClass(playerclass);
}
static ConCommand eo_selectclass("eo_selectclass", CC_EO_SelectClass, "Selects specified class.");

CON_COMMAND_F(eo_pause, "", FCVAR_CHEAT)
{
	CBasePlayer *pPlayer = UTIL_GetCommandClient();
...
CBasePlayer::CBasePlayer( )
{
	...
	m_FuturePlayerClass = CLASS_DEFAULT;

	m_iProtection = NO_PROTECTION;

	m_bDrawLocal = false;

	m_flDeathTime = 0.0f;
	m_flWallDamageInterval = 0.0f;
	m_vWallHitPosition = Vector(0, 0, 0);

	m_bInvulnerable = false;
	m_iFrozenRef = 0;
	m_LastArmourDamage = 0.0f;
	m_NextRechargeUpdate = 0.0f;
	m_iBlinkHealth = 0;
	m_flBlinkStart = 0.0f;
	m_flBlinkCooldown = 0.0f;
	m_SlowdownIndex = 0;
	m_bWantsOverlord = true;
	m_SpawnTime = 0.0f;
	m_DeathNode = NULL;
	m_flCanSpawnTime = 0.0f;
...
int CBasePlayer::ShouldTransmit( const CCheckTransmitInfo *pInfo )
{
	...
	// Always transmit the Ov for health etc. functions
	if((IsOverlord() || IsRebel()) && IsAlive())
		return FL_EDICT_ALWAYS;

	// Always transmit alive players
	/*if(IsAlive() && (IsRebel() || IsOverlord()))
		return FL_EDICT_ALWAYS;*/
...
bool CBasePlayer::WantsLagCompensationOnEntity( const CBaseEntity * pEnt, const CUserCmd *pCmd, const CBitVec<MAX_EDICTS> *pEntityTransmitBits ) const
{
	//Tony; only check teams in teamplay
	if ( gpGlobals->teamplay )
	{
	// Team members shouldn't be adjusted unless friendly fire is on.
	if ( !friendlyfire.GetInt() && pEnt->GetTeamNumber() == GetTeamNumber() )
		return false;
	}

	// If this entity hasn't been transmitted to us and acked, then don't bother lag compensating it.
	if ( pEntityTransmitBits && !pEntityTransmitBits->Get( pEnt->entindex() ) )
		return false;

	const Vector &vMyOrigin = GetAbsOrigin();
	const Vector &vHisOrigin = pEnt->GetAbsOrigin();

	// get max distance player could have moved within max lag compensation time, 
	// multiply by 1.5 to to avoid "dead zones"  (sqrt(2) would be the exact value)

	const CBasePlayer * pPlayer = ToBasePlayer(pEnt);
...
void CBasePlayer::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr )
{
	if ( m_takedamage )
	{
		...
		if(info.GetAttacker() && info.GetAttacker()->IsPlayer())
		{
			...
			{
				if(IsOverlord())
					info.ScaleDamage( eo_sk_overlord_head.GetFloat() );
				else
...
int CBasePlayer::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
	...
	// Only against non-generic damage!
	if(IsInvulnerable() && (bitsDamage != DMG_GENERIC))
		return 0;
	...
	// Armor. 
	// Only against player damage, stops all damage
	if ( !(info.GetDamageType() & (DMG_FALL | DMG_DROWN | DMG_POISON | DMG_RADIATION)) && // armor doesn't protect against fall or drown damage!
		(info.GetAttacker() && info.GetAttacker()->IsPlayer() && GET_OVERLORD_DATA->IsInWarMode())) 
	
	{
		...
		m_LastArmourDamage = gpGlobals->curtime;
		m_NextRechargeUpdate = 0.0f;
	}
...
bool CBasePlayer::IsValidObserverTarget(CBaseEntity * target)
{
	...
	/* Don't spec observers or players who haven't picked a class yet
 	if ( player->IsObserver() )
		...
		// Don't spectate overlord
	if( player->IsOverlord())
		return false;
...
void CBasePlayer::RechargeArmour()
{
	float start = m_LastArmourDamage + ARMOUR_RECHARGE_DELAY ;
	if(start > gpGlobals->curtime)
		return;

	if(m_NextRechargeUpdate == 0.0f && ArmorValue() < SpawnArmorValue())
	{
		m_NextRechargeUpdate = gpGlobals->curtime + ARMOUR_RECHARGE_TIME;
	}
	else if(m_NextRechargeUpdate > gpGlobals->curtime)
		return;

	float diff = gpGlobals->curtime - start;

	float ratio = diff/ARMOUR_TIME_CAP;
	...
	if(ratio >= 1.0f)
	{
		add = ARMOUR_RECHARGE_AMOUNT;
	}
	else
	{
		add = ceil(ARMOUR_RECHARGE_AMOUNT * ratio);
	}

	int maxarmour = SpawnArmorValue();

	int armour = ArmorValue() + add;

	if(armour >= maxarmour)
		armour = maxarmour;

	SetArmorValue(armour);

	m_NextRechargeUpdate = gpGlobals->curtime + ARMOUR_RECHARGE_TIME;
}


void CBasePlayer::CheckForBlink()
{

	if(!IsRebel() || !IsAlive())
		return;

	const float teleportTime = eo_blink_time.GetFloat();
	const int slowdown = eo_blink_slowdown.GetFloat();

	//DevMsg("Blink pressed\n");
	if(m_nButtons & IN_BLINK)
	{
		if(m_flBlinkCooldown >= gpGlobals->curtime)
		{
			float perc = (m_flBlinkCooldown - gpGlobals->curtime)/eo_blink_cooldown.GetFloat();
		...
		if(m_flBlinkStart == 0.0f)
		{
			// Start blink
			m_iBlinkHealth = GetHealth();
			m_flBlinkStart = gpGlobals->curtime;
			m_iBlinkSlowdown = AddSlowdown(slowdown, teleportTime, this, true);
		}
		else
		{
			// Continue blink
			if(GetHealth() < m_iBlinkHealth)
			{
				// Health cancel
				m_flBlinkStart = 0.0f;
				m_flBlinkCooldown = gpGlobals->curtime + 2.0f;
				m_iBlinkHealth = 0;
				RemoveSlowdown(m_iBlinkSlowdown);
				return;
			}

			float perc = 0.0f;
			if((m_flBlinkStart + teleportTime) <= gpGlobals->curtime)
			{
				perc = 1.0f;

				/* Teleport */
				COverlordPathNode * pLast = COverlordPathNode::GetMostPopulatedNode();

				if(pLast)
				{
					pLast->TeleportPlayer(this);
					m_flBlinkCooldown = gpGlobals->curtime + eo_blink_cooldown.GetFloat();
					m_flBlinkStart = 0.0f;
				}
				else
					m_flBlinkCooldown = gpGlobals->curtime + 2.0f;
			...
			{
				perc = (gpGlobals->curtime - m_flBlinkStart)/teleportTime;
			}
	...
	else
	{
		...
		if(m_flBlinkStart != 0.0f)
		{
			...
			m_flBlinkStart = 0.0f;
			m_iBlinkHealth = 0;
			m_flBlinkCooldown = gpGlobals->curtime + 1.0f;
			RemoveSlowdown(m_iBlinkSlowdown);
		}
	}
...
void CBasePlayer::EnableProtection(int iProtect)
{
	m_iProtection |= iProtect;
}

void CBasePlayer::DisableProtection(int iProtect)
{
	m_iProtection &= ~iProtect;
}

void CBasePlayer::SetProtection(int iProtect)
{
	m_iProtection = iProtect;
}

void CBasePlayer::ResolveSelectClass(PlayerClass_t playerclass)
{
	if(!m_bCanSwitchClass /*|| IsOverlord()*/)
	{
		SetFuturePlayerClass(playerclass);
	}
	else
	{
		SwitchToClass(playerclass);
	}
...
ConVar eo_invisibility_alpha("eo_invisibility_alpha", "55", FCVAR_REPLICATED | FCVAR_CHEAT);

void CBasePlayer::MakeInvisible(bool bExternal)
{
	const int INVIS_ALPHA = eo_invisibility_alpha.GetInt();

	SetRenderMode(kRenderTransColor);
	SetRenderColorA(INVIS_ALPHA);
	
	//AddEffects(EF_NODRAW);
	//SetRenderColorA(0);
	//SetRenderMode(kRenderTransAlpha);
	m_bInvisible = INVIS_ON;
	if(bExternal)
		m_bInvisible |= INVIS_EXTERNAL;
	...
	for(int i = 0; i < MAX_WEAPONS; i++)
	{
		...
		pWeapon->SetRenderColorA(INVIS_ALPHA);
...
void CBasePlayer::UnmakeInvisible()
{
	SetRenderColorA(255);
...
void CBasePlayer::IncreaseOverlordDamage(int damage)
{
	if(HL2MPRules()->IsRestarting())
		return;

	m_DamageToOverlord += damage; 

	// Don't let it go below 0
	if(m_DamageToOverlord < 0)
		m_DamageToOverlord = 0;
}

void CBasePlayer::SwitchToClass(PlayerClass_t playerclass)
{
	...
	// We need this to stop the "spawned at the beginning" bug
	ResolveClassFeatures();
	//Spawn();
	
}

void CBasePlayer::GiveClassWeapons()
{
	if(IsRebel())
	{
		GiveAmmo( 200,	"Pistol");
		//GiveNamedItem( "weapon_blinker" );
		...
		else if (GetPlayerClass() == CLASS_HACKER)
		{
			...
			GiveNamedItem( "weapon_hackdevice" );
		}
		else if (GetPlayerClass() == CLASS_STEALTH)
		{
			GiveAmmo( 20,	"SniperRound");
			GiveAmmo( 1, "EMPGrenade" );

			GiveNamedItem("weapon_disabler");
	...
	else if(IsOverlord())
	{
		...
		GiveNamedItem( "weapon_haste");		

		if(GetPlayerClass() == CLASS_TANK)
		{
			GiveNamedItem( "weapon_minigun" );
			// GiveNamedItem("weapon_launcher");
		}
		else if(GetPlayerClass() == CLASS_PHASING)
		{
			GiveNamedItem( "weapon_overshield" );
			GiveNamedItem( "weapon_teleport" );
			GiveNamedItem( "weapon_stasis" );
		}
		else if(GetPlayerClass() == CLASS_TECH)
		{
		}
...
void CBasePlayer::ResetOverlordStats()
{
	ResetDamageToOverlord();
...
void CBasePlayer::ResolveClassFeatures()
{
	if(IsOverlord())
	{		
		m_iMaxHealth = max(OVERLORD_MAX_HEALTH, m_iMaxHealth);
	}
	else if (IsRebel())
	{
		// Health settings
		if(GetPlayerClass() == CLASS_ASSAULT)
		{
			m_iMaxHealth = ASSAULT_MAX_HEALTH;
		}
		else if(GetPlayerClass() == CLASS_PSION)
		{
			m_iMaxHealth = PSION_MAX_HEALTH;
		}
		else if(GetPlayerClass() == CLASS_HACKER)
		{
			m_iMaxHealth = HACKER_MAX_HEALTH;
		}
		else if(GetPlayerClass() == CLASS_STEALTH)
		{
			m_iMaxHealth = STEALTH_MAX_HEALTH;
		}
		else
		{
			m_iMaxHealth = 100;
	...
	{
		m_iMaxHealth = CONVERTED_MAX_HEALTH;
	}
...
{						
	...
	if ((m_nButtons & IN_JUMP) && (!IsOverlord() || !pData->IsInVehicle()))
	{
...
void CBasePlayer::PostThink()
{
	...
	// Handle the combined blink/stasis key
	if(IsRebel())
		CheckForBlink();
	else if(IsOverlord() && (m_afButtonPressed & IN_BLINK))
		engine->ClientCommand(edict(), "eo_poweruppanel_show");


	// Helper block, will let us send quickly info to the
	// client whether we can spawn
	if(!IsAlive() || (GetTeamNumber() == TEAM_SPECTATOR))
	{
		m_bCanSpawn = CanSpawn();
	}
...
void CBasePlayer::Spawn( void )
{
	...
	// Reset damage dealth to the overlord when spawned
	//if(!IsOverlord())
		//ResetDamageToOverlord();

	// Stop drawing local model (camera issue)
	SetDrawLocalModel(false);
 ...
 // only preserve the shadow flag
	...
	// Reset our invulnerable status
	SetInvulnerable(false);

	SetMaxSpeed(HL2_NORM_SPEED);

	if(m_SlowDowns.Size() > 0)
		m_SlowDowns.Purge();
...
bool CBasePlayer::ClientCommand( const CCommand &args )
{
	...
	else if ( stricmp( cmd, "showclassmenu" ) == 0 )
	{
		if(IsOverlord())
			return true;
	...
	else if ( stricmp( cmd, "BuildTrap" ) == 0 )
	{
		if(args.ArgC() < 8)
		{
			Warning("Not enough arguments to build the trap.\n");
			return true;
		}

		COverlordData & data = *GET_OVERLORD_DATA;
		if(!IsOverlord() || !data.IsInVehicle())
			return false;

		const int index = atoi(args.Arg(1));
		Vector trapPos(atof(args.Arg(2)), atof(args.Arg(3)), atof(args.Arg(4)));
		const QAngle trapAng(atof(args.Arg(5)), atof(args.Arg(6)), atof(args.Arg(7)));
		const int entindex = atoi(args.Arg(8));


		COverlordTrap * pTrap = static_cast<COverlordTrap*>(CreateEntityByName(COverlordTrap::GetRecord(index).entName));
		
		if(!pTrap)
		{
			Warning("Server-side trap %s not created.\n", COverlordTrap::GetRecord(index).entName);
			return false;
		}
		...
		if(entindex > 0)
		{
			pParent = static_cast<CBaseEntity*>(gEntList.GetNetworkableHandle(entindex).Get());
			if(pParent)
			{
				pTrap->SetParent(pParent);
			}
			else
				Warning("Can't find parent on server!\n");
		}

		pTrap->SetAbsAngles(trapAng);

		// Decide whether those are relative positions or whether they are not
		if(pParent)
		{
			Vector finalPos;
			pParent->EntityToWorldSpace(trapPos, &finalPos);
			trapPos = finalPos;
		}

		pTrap->SetAbsOrigin(trapPos);

		// Just to make sure that our players do not cheat, this should happen very rarely
		error_t error = pTrap->CanBuild();
		if(!error.canBuild)
		{
			Warning("Trap cannot be built server side! Networking issue, bug or cheat. \n Error: %s \n", error.desc);
			UTIL_Remove(pTrap);
			...
			{
				char hint[512];
				Q_strncpy(hint, error.desc, ARRAYSIZE(hint));
		...
		// Moved!
		//pTrap->InitializeTrap();

		data.AddTrap(pTrap);
		data.HandlePowerEvent(EVENT_DYNAMICTRAP, pTrap->GetTrapCost());

		return true;
	}
	else if ( stricmp( cmd, "SwitchToCamera" ) == 0 )
	{
		if(!IsOverlord())
			return false;

		if(!GET_OVERLORD_DATA->IsInVehicle())
			return false;

		const int entindex = atoi(args.Arg(1));

		if(entindex <= 0)
			return false;

		COverlordCamera * pCamera = dynamic_cast<COverlordCamera*>(gEntList.GetNetworkableHandle(entindex).Get());

		if(pCamera)
		{
			GET_OVERLORD_DATA->SwitchToCamera(pCamera);
			return true;
		}

		return true;
	}
	else if ( stricmp( cmd, "DecayTrap" ) == 0 )
	{
		if(!IsOverlord())
			return false;
		...
		COverlordTrap * pTrap = GetOverlordData()->GetTrap(index);

		if(!pTrap)
			return false;

		// Give back resources for traps in initial dormant which haven't been hurt
		if(pTrap->IsInInitialDormant() && pTrap->GetHealth() == pTrap->GetMaxHealth())
		{
			GET_OVERLORD_DATA->HandlePowerEvent(EVENT_DYNAMICTRAP, -pTrap->GetTrapCost());
		}
	...
	else if ( stricmp( cmd, "SetTrapDormant" ) == 0 )
	{
		if(!IsOverlord())
			return false;
		...
		bool dormant = atoi(args.Arg(1)) != 0 ? true : false;

		const int entindex = atoi(args.Arg(2));

		if(entindex <= 0)
			return false;

		COverlordTrap * pTrap = COverlordTrap::EntityToTrap(static_cast<CBaseEntity*>(gEntList.GetNetworkableHandle(entindex).Get()));

		if(pTrap->CanBeDormant())
		{
			pTrap->SetTrapDormant(dormant);
		}

		return true;
	}
	else if( stricmp( cmd, "RestoreDuration" ) == 0 )
	{
		if(!IsOverlord())
			return false;
		...
		COverlordTrap * pTrap = GET_OVERLORD_DATA->GetTrap(index);

		if(!pTrap)
			return false;

		pTrap->RestoreDuration();

		return true;

	}
	else if( stricmp( cmd, "RestoreHealth" ) == 0 )
	{
		if(!IsOverlord())
			return false;
	...
	else if( stricmp( cmd, "Precache" ) == 0 )
	{
		PrecacheModel(args.Arg(1));
		return true;
	}
	else if( stricmp( cmd, "ExitBuildMode" ) == 0)
	{
		if(!IsOverlord() || !GetOverlordData()->IsInVehicle())
			return false;

		GetOverlordData()->SetBuildMode(BL_NONE);
		return true;
	}
	else if( stricmp( cmd, "EnterBuildMode" ) == 0 )
	{
		if(!IsOverlord() || !GetOverlordData()->IsInVehicle())
			return false;

		GetOverlordData()->SetBuildMode(BL_SELECT);
		return true;
	}
	else if( stricmp( cmd, "EnterMoveTrap" ) == 0 )
	{
		if(!IsOverlord() || !GetOverlordData()->IsInVehicle())
			return false;

		GetOverlordData()->SetBuildMode(BL_MOVE);
		return true;
	}
	else if( stricmp( cmd, "EnterRotateTrap" ) == 0 )
	{
		if(!IsOverlord() || !GetOverlordData()->IsInVehicle())
			return false;

		GetOverlordData()->SetBuildMode(BL_ROTATE);
		return true;
	}
	else if( stricmp( cmd, "ToggleOverlordPreference" ) == 0 )
	{
		if(!IsRebel())
			return false;

		m_bWantsOverlord = !m_bWantsOverlord;

		return true;
	}
	else if( stricmp( cmd, "DisableMouseRotation" ) == 0 )
	{
		if(!IsOverlord())
			return false;
	...
	else if( stricmp( cmd, "EnableMouseRotation" ) == 0 )
	{
		if(!IsOverlord())
			return false;
	...
	else if( stricmp( cmd, "powerup" ) == 0)
	{
		if(!IsOverlord() || !GetOverlordData()->IsInVehicle())
			return false;

		COverlordPowerup::SendCommand(args);
...
// If HL2_DLL is defined, then baseflex.cpp already sends these.
		...
		SendPropFloat		( SENDINFO( m_SpawnTime ), 0, SPROP_NOSCALE ),

		SendPropInt			( SENDINFO( m_nWaterLevel ), 2, SPROP_UNSIGNED ),
		SendPropFloat		( SENDINFO( m_flLaggedMovementValue ), 0, SPROP_NOSCALE ),

		SendPropBool		( SENDINFO( m_bDrawLocal)),
		SendPropBool		( SENDINFO( m_bCanSwitchClass)),
		SendPropInt		    ( SENDINFO( m_iBuybacks)),
		SendPropBool		( SENDINFO( m_bWantsOverlord)),
		SendPropBool		( SENDINFO( m_bCanSpawn)),
...
// -------------------------------------------------------------------------------- //
	...
	IMPLEMENT_SERVERCLASS_ST( CBasePlayer, DT_BasePlayer )
		...
		SendPropInt			( SENDINFO( m_bInvisible)),
		SendPropInt			( SENDINFO( m_DamageToOverlord)),
		SendPropInt			( SENDINFO( m_iProtection)),
		SendPropInt			( SENDINFO( m_PlayerClass ) ),
		SendPropInt			( SENDINFO( m_iRoundDeaths) ),
...
int CBasePlayer::AddSlowdown(float amount, float length, CBaseEntity * pCaller /* = NULL*/, bool bFromMax /*= false*/,
							  bool bToHead /*= false*/)
{
	// Stores our index values
	SlowdownRecord rec;
	rec.m_Index = m_SlowdownIndex;
	m_SlowdownIndex++;
	rec.m_Amount = amount;
	if(length >= 0.0f)
		rec.m_flEndTime = gpGlobals->curtime + length;
	else
		rec.m_flEndTime = -1.0f;

	rec.m_hEntity = pCaller;
	rec.m_bFromMax = bFromMax;

	if(pCaller)
		rec.m_HadEntity = true;
	else
		rec.m_HadEntity = false;

	if(!bToHead)
		m_SlowDowns.AddToTail(rec);
	else
		m_SlowDowns.AddToHead(rec);
...
void CBasePlayer::UpdateSlowdown(int index, float length)
{
	SlowdownRecord & rec = GetSlowdown(index);
...
int CBasePlayer::UpdateSlowdown(float amount, float length, CBaseEntity * pCaller /* = NULL*/, bool bFromMax /*= false*/)
{
	for(int i = 0; i < m_SlowDowns.Count(); i++)
	{
		SlowdownRecord & rec = m_SlowDowns[i];

		if((rec.m_Amount == amount) && (rec.m_hEntity == pCaller) && (rec.m_bFromMax == bFromMax))
		{
			rec.m_flEndTime = gpGlobals->curtime + length;
			UpdateSlowdowns();
			return rec.m_Index;
...
bool CBasePlayer::CanSpawn() const
{
	if(!IsSpawningEnabled())
		return false;

	COverlordPowerup * pPowerup = COverlordPowerup::FindPowerupByName("stasis");

	if(pPowerup && pPowerup->IsActive())
		return false;

	if(GetOverlordData()->GetSpawnsAmount() <= 0)
		return false;

	if(GetTeamNumber() != TEAM_SPECTATOR)
		return false;

	if(m_flCanSpawnTime > gpGlobals->curtime)
		return true;

	COverlordPathNode * pSpawn = COverlordPathNode::GetSpawnNode();
	
	if(!pSpawn)
	{
		Warning("No spawn node\n");
		//DevMsg("No spawn node: false\n");
		return false;
	}
	...
	if(!pLast)
	{
		Warning("No last node\n");
		//DevMsg("No last node: false\n");
		return false;
	}
	...
	if(pLast->CanSpawnNode())
	{
		//DevMsg("!!!CanSpawnNode check on pLast!!!\n");

		if(!m_DeathNode)
		{
			//DevMsg("No death node: true\n");
			return false;
		}

		if(m_DeathNode)
		{
			if(m_DeathNode->CanSpawnNode())
			{
				//DevMsg("CanSpawnNode: true\n");
				return true;
			}
			else
			{
				//DevMsg("CanSpawnNode: false\n");
				return false;
			}
		}
	...
	//DevMsg("The end: false\n");
	return false;
}

SlowdownRecord & CBasePlayer::GetSlowdown(int index)
{ 
	static SlowdownRecord invalid;
	for(int i = 0; i < m_SlowDowns.Count(); i++)
	{
		if(m_SlowDowns[i].m_Index == index)
			return m_SlowDowns[i];
	}

	return invalid;
}

const SlowdownRecord & CBasePlayer::GetSlowdown(int index) const
{ 
	static const SlowdownRecord invalid;
	for(int i = 0; i < m_SlowDowns.Count(); i++)
	{
		if(m_SlowDowns[i].m_Index == index)
			return m_SlowDowns[i];
...
bool CBasePlayer::RemoveSlowdown(int index)
{
	for(int i = 0; i < m_SlowDowns.Count(); i++)
	{
		if(m_SlowDowns[i].m_Index == index)
		{
			m_SlowDowns.Remove(i);
			UpdateSlowdowns();
			return true;
		}
	}
...
bool CBasePlayer::RemoveSlowdown(CBaseEntity * pEntity)
{
	if(!pEntity)
		return false;
	...
	for(int i = 0; i < m_SlowDowns.Count(); i++)
	{
		if(m_SlowDowns[i].m_hEntity == pEntity)
		{
			bRemoved = true;
			m_SlowDowns.Remove(i);
			i--;
...
bool CBasePlayer::HasSlowdown(CBaseEntity * pEntity) const
{
	if(!pEntity)
		return false;
...
void CBasePlayer::UpdateSlowdowns()
{
	//if(m_SlowDowns.Count() <= 0)
	//	return;

	// First get rid of all slowdowns that are "dead"
	for(int i = 0; i < m_SlowDowns.Count(); i++)
	{
		if((m_SlowDowns[i].m_flEndTime >= 0.0f) && (m_SlowDowns[i].m_flEndTime <= gpGlobals->curtime))
		{
			m_SlowDowns.Remove(i);
			i--;
		}

		if(m_SlowDowns[i].m_hEntity == NULL && m_SlowDowns[i].m_HadEntity)
		{
			m_SlowDowns.Remove(i);
			i--;
		}
	...
	//DevMsg("Slowdown update\n");
	// Now calculate all of them
	const int topSpeed = GetCurrentTopSpeed();
	int maxSpeed = topSpeed;
	int highestFromMax = 0;
	for(int i = 0; i < m_SlowDowns.Count(); i++)
	{
		const SlowdownRecord & record = m_SlowDowns[i];
		if(record.m_bFromMax)
		{
			if(highestFromMax < record.m_Amount)
				highestFromMax = record.m_Amount;

			continue;
		}

		maxSpeed = maxSpeed - record.m_Amount;
	}


	// Subtract "from max" slowdowns
	if(highestFromMax != 0)
	{
		if(maxSpeed >= topSpeed)
		{
			maxSpeed -= highestFromMax;
		}
		else
		{
			int newSpeed = topSpeed - highestFromMax;
...
void CBasePlayer::RemoveFrozenRef()
{
	m_iFrozenRef--;
...
int CBasePlayer::SpawnArmorValue( void ) const
{
	if(IsOverlord())
	{
		if(GetPlayerClass() == CLASS_TANK)
			return TANK_MAX_ARMOR;
		else if(GetPlayerClass() == CLASS_PHASING)
			return PHASING_MAX_ARMOR;
		else if(GetPlayerClass() == CLASS_TECH)
			return TECH_MAX_ARMOR;
	}
	else if(IsConverted())
		return CONVERTED_MAX_ARMOR;
	else if(IsRebel())
	{
		if(GetPlayerClass() == CLASS_ASSAULT)
			return ASSAULT_MAX_ARMOR;
		else if(GetPlayerClass() == CLASS_PSION)
			return PSION_MAX_ARMOR;
		else if(GetPlayerClass() == CLASS_STEALTH)
			return STEALTH_MAX_ARMOR;
		else if(GetPlayerClass() == CLASS_HACKER)
			return HACKER_MAX_ARMOR;
	}
...
CON_COMMAND(eo_freeroam, "Freeroam")
{
	CBasePlayer * pPlayer = UTIL_GetCommandClient();
	if(!pPlayer || !pPlayer->IsOverlord())
		return;
	...
	Vector pos;
	QAngle angle;
	// Create freeroam by default
	if(args.ArgC() < 7)
	{
		COverlordCamera * cam = pData->GetCamera();

		if(!pData->IsInVehicle() || !cam)
			return;
		
		pos = cam->GetAbsOrigin();
		angle = cam->GetAbsAngles();
	}
	else
	{
		// Use passed arguments
		// Legend:
		// eo_freeroam <pos_x> <pos_y> <pos_z> <ang_x> <ang_y> <ang_z>
		pos = Vector(atof(args[1]), atof(args[2]), atof(args[3]));
		angle = QAngle(atof(args[4]), atof(args[5]), atof(args[6]));
	}

	// Destroy current freeroam camera
	COverlordCamera * pPrevious = static_cast<COverlordCamera*>(gEntList.FindEntityByClassname(NULL, "overlord_freeroam"));

	if(pPrevious)
		UTIL_Remove(pPrevious);

	COverlordCamera * pCamera = static_cast<COverlordCamera*>(CreateEntityByName("overlord_freeroam"));

	if(pCamera)
	{
		pCamera->SetAbsOrigin(pos);
		pCamera->SetAbsAngles(angle);
		pCamera->Spawn();

		pData->SwitchToCamera(pCamera);
	}
	else
		Warning("Cannot create freeroam!\n");

}

CON_COMMAND(eo_lastnode, "")
{
	COverlordPathNode * pLast = COverlordPathNode::GetLastNode() ? COverlordPathNode::GetLastNode()->GetNextNode() : NULL;

	if(!pLast)
		return;

	Vector position = pLast->WorldSpaceCenter() + Vector(0, 0, 8);
	QAngle angle;

	if(pLast->GetNextNode())
	{
		Vector dir = pLast->GetPreviousNode()->WorldSpaceCenter() - pLast->WorldSpaceCenter();
		dir.z = 0.0f;
		VectorNormalize(dir);
	...
	{
		angle = pLast->GetAbsAngles();
	}

	COverlordData & data = *GetOverlordData();
	COverlordCamera * pCam = data.GetCamera();

	if(!pCam || Q_stricmp(pCam->GetClassname(), "overlord_freeroam"))
	{
		COverlordCamera * pCamera = static_cast<COverlordCamera*>(CreateEntityByName("overlord_freeroam"));

		if(pCamera)
		{
			pCamera->SetAbsOrigin(position);
			pCamera->SetAbsAngles(angle);
			pCamera->Spawn();
			data.SwitchToCamera(pCamera);
		}
	}
	else if(pCam && !Q_stricmp(pCam->GetClassname(), "overlord_freeroam"))
	{
		pCam->SetAbsOrigin(position);
		pCam->SetAbsAngles(angle);
	}
}

CON_COMMAND(eo_respawn, "")
{
	CBasePlayer * pPlayer = UTIL_GetCommandClient();
