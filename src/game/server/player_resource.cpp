...
ConVar eo_playtest_show_score("eo_playtest_show_score", "1", FCVAR_REPLICATED 
#ifdef OVER_PLAYTEST							  
				| FCVAR_HIDDEN
#endif
							  );
...
//	SendPropArray( SendPropInt( SENDINFO_ARRAY(m_iPacketloss), 7, SPROP_UNSIGNED ), m_iPacketloss ),
	...
	SendPropArray3( SENDINFO_ARRAY3(m_Class), SendPropInt( SENDINFO_ARRAY(m_Class))),
	SendPropArray3( SENDINFO_ARRAY3(m_bWantsOverlord), SendPropBool( SENDINFO_ARRAY(m_bWantsOverlord))),
END_SEND_TABLE()
...
void CPlayerResource::UpdatePlayerData( void )
{
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		if ( pPlayer && pPlayer->IsConnected() )
		{
			...
			if(eo_playtest_show_score.GetBool() && !pPlayer->IsOverlord())
				m_iScore.Set( i, pPlayer->DamageToOverlord() );
			else
			...
			m_Class.Set(i, pPlayer->GetPlayerClass());
			m_bWantsOverlord.Set(i, pPlayer->WantsToBeOverlord());
