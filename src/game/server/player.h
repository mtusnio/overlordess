...
struct SlowdownRecord
{
	...
	SlowdownRecord(int index, float amount, float endtime, CBaseEntity * pEnt, bool bfrommax)
	{
		m_Index = index;
		m_Amount = amount;
		m_flEndTime = endtime;
...
#define	HL2_WALK_SPEED hl2_walkspeed.GetInt()
#define	HL2_NORM_SPEED hl2_normspeed.GetInt()
#define	HL2_SPRINT_SPEED hl2_sprintspeed.GetInt()
...
public:
	...
	virtual void			SetLastWallHitPosition(Vector pos) { m_vWallHitPosition = pos; };
	virtual void			SetWallDamageInterval(float flTime) { m_flWallDamageInterval = flTime; };
	virtual float		    GetWallDamageInterval() const { return m_flWallDamageInterval; };
	virtual Vector			GetLastWallHitPosition() const { return m_vWallHitPosition; };
	...
	// Eye position is used for VIEW position, not actual eye location. Hence
	// we need this function
	virtual Vector			RealEyePosition();

	const QAngle			&EyeAngles( );
	virtual QAngle          RealEyeAngles();
	...
	virtual void			CreateRagdollEntity( int damagetype = 0 ) { return; }
...
public:
	...
	// Both return the id of the slowdown
	int    AddSlowdown(float amount, float length = -1.0f, CBaseEntity * pCaller = NULL,
		bool bFromMax = false, bool bToHead = false);
	void   UpdateSlowdown(int index, float amount);
	int    UpdateSlowdown(float amount, float length = -1.0f, CBaseEntity * pCaller = NULL,
		bool bFromMax = false);

	virtual int GetCurrentTopSpeed() const { return HL2_NORM_SPEED; };

	virtual void				SetDeathNode(COverlordPathNode * node) { m_DeathNode = node; };
	virtual COverlordPathNode * GetDeathNode() const { return m_DeathNode; };
	virtual bool CanSpawn() const;
	virtual void SetCanSpawnTime(float flTime) { m_flCanSpawnTime = gpGlobals->curtime + flTime; };
	virtual void SetSpawningEnabled(bool bEnabled) { m_bSpawningEnabled = bEnabled; };
	virtual bool IsSpawningEnabled() const { return m_bSpawningEnabled; };

	const SlowdownRecord & GetSlowdown(int index) const;
	SlowdownRecord & GetSlowdown(int index);

	bool		   RemoveSlowdown(int index);
	bool		   RemoveSlowdown(CBaseEntity * pEntity);

	bool    HasSlowdown(CBaseEntity * pEntity) const;
	void    UpdateSlowdowns();

	void    AddFrozenRef();
	void    RemoveFrozenRef();
	int     GetFrozenRef() const { return m_iFrozenRef; };
	...
	virtual void			KillArmour();
private:
	int   m_iFrozenRef;
	float m_flWallDamageInterval;
	Vector m_vWallHitPosition;
	...
	// Last damaged by a player
	float				m_LastArmourDamage;
	float				m_NextRechargeUpdate;

	// Blinker settings
	int					m_iBlinkHealth;
	float				m_flBlinkStart;
	float				m_flBlinkCooldown;
	int					m_iBlinkSlowdown;

	CUtlVector<SlowdownRecord> m_SlowDowns;
	int						   m_SlowdownIndex;

	CHandle<COverlordPathNode> m_DeathNode;
	float	m_flCanSpawnTime;
	CNetworkVar(bool, m_bCanSpawn);
public:
	
	// Used by a couple of weapons that calculate extra damage
	int GetLastDamageAmount() const { return m_lastDamageAmount; };
...
protected:
	...
	void					CalcOverlordView(Vector &eyeOrigin, QAngle &eyeAngles, float &fov);
...
public:
	...
	//virtual void			SwitchToCamera(COverlordCamera * cam);
	//virtual void			SwitchFromCamera();
	virtual bool			IsPlayingRebel() const;
	
	void			EnableProtection(int iProtect);
	void            DisableProtection(int iProtect);
	void            SetProtection(int iProtect);
	int				GetProtection() const { return m_iProtection.Get(); };
	
	// Class system
	PlayerClass_t GetPlayerClass(void) const;
	const char *  GetPlayerClassName(void) const;
	void		  SetPlayerClass(PlayerClass_t playerclass) { m_PlayerClass = playerclass; };
	// has the player selected a class or just spawned with some default one
	bool		  HasSelectedClass() const { return m_bCanSwitchClass; };
	
	PlayerClass_t GetFuturePlayerClass(void) const { return m_FuturePlayerClass; };
	void		  SetFuturePlayerClass(PlayerClass_t playerclass) { m_FuturePlayerClass = playerclass; };

	virtual	void  ResolveSelectClass(PlayerClass_t playerclass);		  

	const char *  GetAreaName() const { return &m_iszAreaName[0]; };
	virtual void  SetAreaName(const char * name) { Q_strncpy(m_iszAreaName, name, MAX_AREA_NAME_SIZE); };

	virtual void  MakeInvisible(bool bExternal = false);
	virtual void  UnmakeInvisible();
	virtual bool  IsInvisible() const;

	void		  SetDrawLocalModel(bool draw) { m_bDrawLocal = draw; };
	
	// At the moment damage = points, we earn them not only by damaging the Ov
	virtual void  IncreaseOverlordDamage(int damage);
	virtual int	  DamageToOverlord() const { return m_DamageToOverlord; };
	virtual void  ResetDamageToOverlord() { m_DamageToOverlord = 0; };
	virtual int   GetDeathsThisRound() const { return m_iRoundDeaths.Get(); };;
	virtual int   GetPowerPerKill() const;

	virtual bool  IsInvulnerable() const { return m_bInvulnerable; };
	virtual void  SetInvulnerable(bool bInv) { m_bInvulnerable = bInv; };

	bool		  WantsToBeOverlord() const { return m_bWantsOverlord.Get(); };
protected:
	virtual void		  SwitchToClass(const PlayerClass_t playerclass);
	virtual	void		  GiveClassWeapons();
	virtual void		  ResolveClassFeatures();
	virtual void		  ResetOverlordStats();

	CNetworkVar(int, m_bInvisible);
	CNetworkVar(int, m_iProtection);
	CNetworkVar(int, m_DamageToOverlord);
	CNetworkVar(float, m_SpawnTime);

	CNetworkVar(PlayerClass_t, m_PlayerClass);
	CNetworkVar(bool, m_bDrawLocal);

	PlayerClass_t m_FuturePlayerClass;
	CNetworkVar(bool, m_bCanSwitchClass);
	
	CNetworkVar(int, m_iBuybacks);
	CNetworkVar(bool, m_bWantsOverlord);

private:
	bool m_autoKickDisabled;

	char m_iszAreaName[MAX_AREA_NAME_SIZE];
	CNetworkVar(int, m_iRoundDeaths);
	//bool m_bCameraMenu;
